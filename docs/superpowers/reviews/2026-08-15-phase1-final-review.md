...
## 复审记录(2026-08-15,修复后 scoped 复审)

修复提交:`7777b67`(3 Major + 高影响 Minor)、`52f3d97`(sdk min_version 回 2.7.1)。

- **M1/M2/M3**:✅ 全部修复有效,未引入新问题(动态 WHERE 仅拼固定片段、参数化保持)
- 已修 Minor:#5/#6/#7/#8/#10/#11/#12/#13/#16 均有效落地
- 未修 Minor 判定:#1-#4 授权偏离与二期项(接受)、#9 自然日去重(默认 24h 无影响,接受)、#14 plugin.py 接线层单测缺口(二期)、#15 min_version 2.7.1(实机 SDK 即为 2.7.1,保持)
- 测试:94 个全过(92 + 新增 2)
