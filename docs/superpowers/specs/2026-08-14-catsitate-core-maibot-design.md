...
| 缓存 | 注入合并单条+版本化;注入块前插到动态区之前(两档实测定档);旁路 LLM 有预算与计数;测量用主程序日志/统计 |
...
> 实现期 spike 清单:① 插件加载器是否支持子包(插件目录加入 `sys.path` 后绝对导入 `catsitate_core.*`),不支持则退化为同目录平铺多模块;② `before_request` Hook 中 `items` 的插入语义——构造合法 context item(`item_schema_version`)插入历史之后的容忍度,决定注入插入点档 1/档 2。
...
- **注入位置(缓存关键)**:主程序每轮请求含"当前时间"等必变动态消息,尾部追加的注入永远进不了缓存前缀。因此注入采用**前插**:把合并后的稳定注入块插到动态区之前,使时间消息等必变内容自然落到注入块之后(不移动主程序自有消息)。插入点两档,由实测命中率定档:
  - 档 1(最稳):紧贴历史之后、主程序动态注入(`injected_user_messages`)之前——需 spike 验证主程序容忍度;
  - 档 2(保守):时间消息之前、主程序动态注入之后(默认档,仅获得跨轮复用收益);
  - 插入失败或 spike 证伪档 1 时回退追加尾部(仅日志,不阻塞主链路);
- **注入合并**:所有模块的注入内容合并为**至多一条** user 消息(单条 `[Catsitate 感知]`,内部小节:环境/好感度/备忘),减少消息条数与缓存边界扰动;
- **注入版本化**:每个小节由 (模块, 内容 key, 内容 hash) 标识,内容未变时字节级复用上一轮渲染结果,保证跨请求稳定;
- 单条注入总长度上限(默认 600 字符,可配),超限按优先级截断(顺序:环境 > 好感度 > 备忘);
...
- **注入(Q8 A+C)**:`maisaka.planner.before_request` 尾部追加当前说话人片段——私聊=对端用户,群聊=当前消息发送者;内容:`[好感度] XXX:等级「熟悉」(累计 42),注记:最近主动关心过你。` + 该等级行为准则短片段(5 级:陌生/熟悉/亲近/挚友/特别);等级与注记变化才更新(低频,缓存友好);
...
**缓存命中优化**(供应商前缀缓存语义下):
- 插件承诺不碰 system prompt 与历史(最长公共前缀不受损);注入块**前插到动态区之前**(见 §4.1 两档插入点),使注入 token 进入可缓存前缀;位于注入块之后的必变内容(时间/tail)仅损失其后少量 token;
- 尾部波动源及其稳定化:环境小节 45 分钟级稳定(天气)、日级稳定(节日);好感度小节仅在等级/注记变化时更新;备忘小节仅在活跃备忘集合变化时更新,条数有上限;
...
- `inject`:enabled、max_inject_chars(默认 600)
