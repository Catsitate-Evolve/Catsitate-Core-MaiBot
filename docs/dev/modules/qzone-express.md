...
生命周期:润色随工具调用即时发生;日记与「醒来日日程生成」同属入睡任务(睡眠期旁路 LLM 与发布 API 均不经消息链,深夜直发);见闻在窗口边界触发(后台任务)。
...
1. **素材只取素材日(入睡日)真实数据**(模板明令不得编造,防日记虚构没发生的事):素材日由调用方传入(`sleep_day`=睡眠窗口起始日)——默认配置下静默入睡必然发生在午夜后,此刻 now 已是次日,素材日若用 now 取日会写成次日;两条入睡任务派发路径(`_enter_sleep`/窗口终点补执行)均传窗口起始日。
   - 蓝本头部:「我的名字是{bot 昵称}」+ 人设(第二人称散文体)+「今天是{年月日}(素材日),回顾一下到现在为止的聊天记录:{时间线}」。
   - **聊天时间线**(`_diary_chat_timeline`):经 `message.get_by_time` **全局**拉素材日 00:00 起全部消息(跨全部聊天流,不限条数;空间虚拟流消息按平台剔除——日记素材=真实聊天);逐条「[HH:MM] 谁:内容」时间序铺开,单条截 100 字尾加"...",总量超 300 条保留最近并标注「(更早的聊天已略)」;bot 标「我」,纯图/表情等无文本消息不进时间线。能力失败**或返回形态异常**(如 success 但缺 messages 键)均显式告警后回退旧逐流取数(get_by_time 不可用时)。
   - 其余素材行:素材日日程活动(只取活动窗口)、素材日到期备忘(前 3 条)、看到的好友动态(seen 表当日 3 条摘要)、当前真实天气(time_aware 快照,无数据/快照过期(超 6 小时)/读取异常时**省略该行,不臆造天气**)、篇幅区间(`diary_word_count_min~max` 配置直接进素材行作字数指导,不做随机化——目标字数随机化违反设计哲学,此形态对齐 diary_plugin `qzone_min/max_word_count` 的配置指导)。
   - 素材尾「日记内容:」收尾作生成引导。
...
- **素材**:近 24h 滚动窗统一锚点(浏览 `recent_seen(days=1)` 与互动 `fav_events_window(now-24h)` 同窗)——浏览侧 seen 表近 1 天动态按注入时间倒序取最新 15 条(「昵称发了「摘要20字」」,纯图以「图片」占位),互动侧全部用户近 24h 好感度事件(`qzone_fav_events`)按时刻升序取尾保留最新 10 条(单条截 40 字)。跨零点窗口结束时昨晚素材自然衔接,见闻语义为「近 24h 滚动印象」而非自然日印象。窗内无素材不生成,保留旧见闻。
...
| 日记天气快照过期(超 6 小时) | 按「无数据」处理,素材行省略——持续拉取失败时旧天气不得无限期当当前天气 |
| 日记聊天素材 get_by_time 失败/形态异常(success 但缺 messages 键) | 显式告警后回退旧逐流取数(不静默按 0 条,防日记误写「今天没和人聊天」);仍无消息返回空串(素材行省略) |
