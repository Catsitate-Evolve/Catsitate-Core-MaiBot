...
- v0.9.1(2026-09-03):**发现层协议校准**(scope=0→2 好友动态流(旧取值实为 2 天小窗口且只见自己动态)/begin 偏移→begintime 游标翻页(旧参数被服务端无视)/解析 appid 回退 appiconid 防窗口边界丢条,见 §3.13),即当前公测版本。
...
- **qzone_reply 全域化(v0.9.0,Q3/Q11)**:移除虚拟流门控,任何聊天流可用;comment_id 锚来源=空间流通知参数行、浏览注入评论区、或 view_friend_feed_detail 结果。目标解析三级:①通知上下文(主评论作者+评论者)②comment_map(浏览/详情登记)③全 miss 显式拒绝+指引「先用 view_friend_feed_detail 查看该说说」——不猜测回退,防 @ 错人。
