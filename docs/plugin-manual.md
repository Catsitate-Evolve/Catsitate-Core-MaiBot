...
- v0.8.1(2026-09-02):联调改进集(日记模板 v5 仿 diary_plugin 蓝本重排/`qzone_like`+`qzone_comment`+`qzone_post` 三工具全域可用+白名单只管非 qzone 工具/view_friend_feeds 返回格式可读性优化/通知消息不再强制触发回复概率,见 §3.13),即当前公测版本。
...
- **日记(M3 表达,入睡任务旁路生成+API 直发+延迟回注)**:入睡时与次日日程生成同属入睡任务——用旁路 LLM(`qzone_diary` 模板,v0.8.1 升 v5 仿 diary_plugin 蓝本重排:编号要求+「书写风格」段+输出卫生,核心=日期天气开头/反流水账/简短口语/个性许可)以 **bot 本人身份**书写(人设前置为素材首段「bot 人设:…」,与表达润色层同一个人设出口),基于**当日真实素材**(今天是几号+日程活动概览+到期备忘+当日空间见闻+聊天时间线+真实天气)生成 80~200 字第一人称日记(每次生成随机目标字数注入「本次目标约 {n} 字」,n∈[80,200],避免定长模板感;模板的「80~200字」区间表述保留),经发布 API(`do_publish`)直发为说说;旁路 LLM 与发布 API 均不经消息链,**不受睡眠拦截(深夜直发)**;睡眠窗口终点未入睡时由 tick 补执行(每窗口至多一篇)。聊天时间线=真实聊天流当日消息按小时分段(bot 消息标「我:」、他人标昵称、单条截 50 字、总量截 20 条;虚拟流消息不含——日记素材=真实聊天);真实天气=当前快照(无数据省略该行——模板同款口径「没给就只写日期」);两者取数失败均告警后省略素材行,不拖垮日记主链路。护栏:模板明令「内容必须完全基于素材里给到的当日事实,不要编造」;空/超 300 字视为输出异常跳过;发布失败不落回注快照(没发出去的日记不该回注成「昨晚发过」的假上下文)。**回注延迟到醒来**:正文+发布时刻存 `qzone_pending_diary.json`,醒态 `sleep_tick` 以 self 消息「我昨晚发布的日记:{前 60 字}」补注进虚拟流(睡眠期注入会被 sleep_gate 拦进回顾缓冲,白注入);补注失败保留快照下轮重试。开关 `diary_enabled`(默认开)。
