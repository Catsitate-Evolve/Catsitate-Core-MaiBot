...
1. **素材只取素材日(入睡日)真实数据**(模板明令不得编造,防日记虚构没发生的事):素材日由调用方传入(`sleep_day`=睡眠窗口起始日)——默认配置下静默入睡必然发生在午夜后,此刻 now 已是次日,素材日若用 now 取日会写成次日;两条入睡任务派发路径(`_enter_sleep`/窗口终点补执行)均传窗口起始日。
   ...
   - 其余素材行:素材日日程活动(只取活动窗口)、素材日到期备忘(前 3 条)、看到的好友动态(seen 表当日 3 条摘要)、当前真实天气(time_aware 快照,无数据/快照过期(超 6 小时)/读取异常时**省略该行,不臆造天气**)、篇幅区间(`diary_word_count_min~max` 配置直接进素材行作字数指导,不做随机化——2026-09-04 用户裁定:目标字数随机化违反设计哲学,对齐 diary_plugin `qzone_min/max_word_count` 的配置指导形态)。
   - 素材尾「日记内容:」收尾作生成引导。
