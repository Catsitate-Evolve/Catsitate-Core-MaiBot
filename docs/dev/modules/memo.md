...
另外,日程注入块行末会附当日到期备忘(≤3 条,按说话人/流过滤);睡醒回顾报告会静态附列睡眠期到期的备忘(不占 LLM 额度,备忘不丢失;跨午夜场景按「入睡日 ∪ 今天」并集取数再过滤 `[入睡时刻, 现在]`,入睡日深夜到期的备忘不丢)。

### 4. 到点提醒(双路)

- **日程收录(主路)**:醒来日日程生成时(`_generate_tomorrow_schedule`,目标日=睡眠窗口 end 所在自然日,由调用方显式传入),目标日到期备忘(`due_on`)作为 prompt 素材传入,由 LLM 排进作息(如在提醒时刻安排活动/计划发言)。当天持有 LLM 生成的日程时,兜底 tick 主动让位不重复提醒。
...
| 睡眠期 | 兜底提醒不执行;睡眠期到期的备忘在睡醒回顾报告中静态补列(入睡日 ∪ 今天并集,跨午夜不丢) |
