...
9. **主动问候统一(联调裁定 2026-08-16,原 2.3 与 greeting 合并)**:主动问候只保留**私聊通道**,且强制要求私聊流存在;仅在 greeting 窗口触发,窗口**起点即触发**;触发条件 = 存在「特别」等级的人 + 该人存在私聊流 + 当日未问候过该人;满足则对其私聊流 trigger 问候语境(每人每日最多一次),否则不触发——**不群内替代**。删除 `greet_threshold_level` 与 `private_threshold_level` 配置(问候门槛与特别等级绑定)。
...
- **触发条件**(全部满足):(1) 存在「特别」等级的人(全表唯一);(2) 该人存在私聊流(chat.get_all_streams 中的非群流);(3) 当日未问候过该人(`_greet_sent` 每人每日一次,计 1)。
- **行为**:对其私聊流 `maisaka.proactive.trigger`(问候语境指示 prompt,含日程活动与全天概览,问候话术由主程序生成),发送成功计 1(受 `daily_speak_limit` 约束)。不满足条件则不触发、不群内替代。
- 睡眠/静默期跳过;被拦截/失败显式日志。
- **测试**:特别者存在且无私聊流 → 不触发;无私聊流 → 不触发;每日一次限制;睡眠期跳过;窗口起点触发(同窗口不重复)。
