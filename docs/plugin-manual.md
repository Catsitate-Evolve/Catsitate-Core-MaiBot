...
- `QQ空间表达润色失败,以草稿直发` / `QQ空间表达润色返回空文本,以草稿直发`(润色层失败回退:草稿即模型完整表达,显式告警不阻断);`QQ空间表达润色超长(N 字>上限 …),这次改短一些重新润色` / `QQ空间表达润色仍超长(N 字),截断至 N 字`(护栏路径,不失败)
