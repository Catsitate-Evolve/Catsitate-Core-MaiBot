...
## v0.8.0(2026-09-01) M3 表达:说说发布/日记/表达生成层/见闻/点赞通知

**说说发布与表达生成(两段式)**

- **`qzone_post` 工具(说说发布)**:bot 想分享自己的心情/见闻时自主调用发布说说(仅虚拟流会话可用,与三互动工具同款硬门控;无频控——是否发、表达什么完全由模型决定)。发布失败不重试,登录态失效自动作废 cookie 下轮重取。
- **表达生成层(评论/回复/发布共用)**:三动作工具改两段式——模型只填 `reply_reference`(表达方向:要点/事实/关系/语气倾向)与 `reply_style`(篇幅),正文由带完整 bot 人设的旁路 LLM 生成(新模板 `qzone_expression`),空间动作与真实聊天共享同一个人设出口,不再是 planner 的「转述腔」。评论带目标说说正文与近几条评论素材(防复读);护栏:评论/回复 ≤200 字、说说 ≤500 字(超长带硬约束重生成一次,仍超截断),生成失败显式报错可重试,不静默兜底。
- **说说发布主动触发**:日程窗口新属性 `send_qzone` 标记「会有感而发写说说」的时段——同窗形态(刷+发)在首轮浏览完成后触发(刚看完好友动态,分享有上下文),仅发形态在窗口开始即触发(忙里偷闲);经 `maisaka.proactive.trigger` 注入分享指示,想发就调 qzone_post,不想发保持沉默。冷启动种子自举:虚拟流会话尚未诞生时注入一条不触发决策轮的种子消息仅建会话后重试一次。
...
- **场景 prompt v4**(`qzone_scene`):对齐两段式参数与点赞通知——评论/回复/发布的 `reply_reference` 填表达方向、正文按口吻生成,互动通知含「赞了你」,说说ID归属含点赞工具;内置/`prompt_templates/`/兜底常量三处逐字一致。
