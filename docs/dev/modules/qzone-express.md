...
- **见闻**:read_qzone 浏览窗口结束时把当日浏览与互动摘要为一段「空间见闻」,注入真实聊天。
...
1. **素材只取当日真实数据**(模板明令不得编造,防日记虚构没发生的事):
   ...
   - 其余素材行:当日日程活动(只取活动窗口)、到期备忘(前 3 条)、看到的好友动态(seen 表当日 3 条摘要)、当前真实天气(time_aware 快照,无数据/读取异常时**省略该行,不臆造天气**)、目标字数(随机 80~200——每次生成注入随机目标,避免模板化的定长输出)。
   - 素材尾「日记内容:」收尾作生成引导。
2. **生成**:旁路 LLM(模板 `qzone_diary`,指令含日期天气开头/像睡前随手写/反流水账要有重点和感情色彩/第一人称/输出卫生)。
3. **发布**:`client.do_publish` 直发为说说(不经消息链,不受睡眠拦截)。发布前经内容护栏匹配,命中即拦截(不发布不落快照)。登录态失效走同轮自愈(与 qzone_post 同款——入睡任务无用户回执,失败只有日志可见)。不设长度硬上限(长度完全由模板目标字数软约束);**空文本跳过发布**(空日记没有发布意义)。
...
- **素材**:当日已见动态(seen 表近 1 天 15 条,「昵称发了「摘要20字」」)+ 当日好感度事件(fav_events,前 10 条)。当日无素材不生成,保留旧见闻。
- **生成**:旁路 LLM(模板 `qzone_digest` v2:「回想一下今天在QQ空间的事……写成一段空间见闻」,60~150 字,一段话)。
