...
- v0.8.5(2026-09-02):通知死锁修复+润色事实保持+日程 RPC 超时(awaiting 卡死吞新评论的扫描死锁/润色模板 v6 不修改关键事实/schedule_llm_timeout_ms 默认 120000,见 §3.13),即当前公测版本。
...
- **写路径有真实副作用(风险提示)**:评论/回复/点赞/发布说说一经 `qzone_comment`/`qzone_reply`/`qzone_like`/`qzone_post` 调用成功即真实发布到QQ空间(无撤回通路);**是否互动、表达什么方向、是否发说说,完全由模型自主决定**(content 由模型直写、发出前按人设润色;插件只做工程护栏——同说说评论频控上限 3、正文上限 200/500 字(入参校验;润色重润仍超不截断,v0.8.4)、`qzone_reply` 虚拟流会话门控、目标三级解析失败显式拒绝、润色失败以草稿直发不静默);**三动作工具全域可用(v0.8.1)**——真实聊天流里模型同样可能调它们点赞/评论/发说说,关闭 `qzone.enabled` 才是整体停用开关。入睡任务还会每晚自动发布一篇日记(`diary_enabled` 可单独关闭)。`qzone.enabled` 关闭即整体停用(读+写一起);不希望写动作时关闭该开关或停用模块,不存在"只读不禁写"的中间档。
