...
3. **睡眠即日程窗口,日程按活动排列(动态数量)**:全天日程 = **恰好 1 个睡眠窗口 + 1~8 个活动窗口**。睡眠窗口 = **可入睡时间**(静默关=窗口起点直接入睡;静默开=安静满 N 分钟入睡;窗口内晚安判定可入睡;窗口结束=醒来或不再入睡,见 §3.2);活动可挤占睡眠时间——工具/生成排活动与睡眠重叠时,睡眠压缩(入睡推迟、醒来不变),压缩不足最短睡眠则拒绝;默认作息 23:00 睡/7:30 醒为软基准(生成 prompt 提示,LLM 可结合当天活动调整),活动窗口数量与名称由 LLM 生成时自主决定,也可经 `update_schedule` 工具增删改(联调修正:取消固定 4 活动窗口框架);时间段由 LLM 自主排布,窗口不重叠,**允许空白时间**(空白=自由时间)。
...
- **生成时机**:**任何入睡状态切换成功的瞬间**(晚安判定入睡 / 静默关到点入睡 / 静默入睡)生成次日日程(拟人:睡前规划明天);**例外补生成**:睡眠窗口终点未入睡时补生成次日日程(见 3.2,每窗口一次);
