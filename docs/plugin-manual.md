...
- 文档日期:2026-09-01(对应 v0.8.0,QQ空间 M3 表达与修正批次——`qzone_post` 说说发布/表达润色层(`content` 由模型直写,发出前按人设口吻润色)/睡前日记旁路生成+API 直发+醒来延迟回注/self 消息回注/发布主动触发+冷启动种子自举/空间见闻当日摘要/点赞通知(源C)/通知独立于浏览窗口/view_friend_feeds 全域查看与 inspect_image hash 前缀/发现层翻页配置;叠加 v0.7.0 工具驱动架构(评论/楼中楼回复/点赞改为 `qzone_comment`/`qzone_reply`/`qzone_like` 工具直调,网关改 receive 只进不出)/v0.6.0 统一时间线/v0.5.x 互动与统一通知;见 §3.13)
- 适用对象:公测使用方、复审人员
...
| **内容护栏兜底** | 出站内容的**最后一道防线**:可配置正则(`guard.patterns`)硬拦在 LLM 判定之外——不依赖模型自觉,防已知违规模式外发(§3.9.3,默认关)。 |

### 1.3 版本与里程碑

- v0.1.0(2026-08-15):一期联调完成(注入/好感度/备忘录/贴表情/戳一戳/reply 补传/图片重看/时间感知/旁路记账)。
...
- **以下 v0.4.0 ~ v0.9.x 为测试版(未正式发布)**:开发迭代记录,数据格式可能不兼容,升级请到 v1.0.0。
- v0.4.0(测试版,2026-08-30):三期 M1 QQ空间感知(日程浏览窗口内刷好友动态/`qzone-qq` 虚拟流注入/真实聊天见闻摘要,见 §3.13)。
- v0.5.0(测试版,2026-08-31):三期 M2 QQ空间互动(出站评论路由/`qzone_like` 点赞工具/好感度显式事件/memo 按人重构,见 §3.13)。
- v0.5.1(测试版,2026-08-31):三期 M2.1 QQ空间统一通知通道(评论轮询重构为高频双源检测+P1 通知插队+阅读顺序新→旧+工具双向隔离,见 §3.13)。
- v0.5.2(测试版,2026-08-31):QQ空间二轮深度审查修复(通知注入重试上限/事件同日去重/结算候选并集等,见 CHANGELOG)。
- v0.6.0(测试版,2026-08-31):三期 M3 QQ空间统一时间线架构(浏览流发现层+充实层重构/通知源B搭便车/API 量与好友数无关,见 §3.13)。
- v0.7.0(测试版,2026-09-01):QQ空间**工具驱动架构**(出站意图系统删除,`qzone_like`/`qzone_comment`/`qzone_reply` 三工具直调+`FeedContextRegistry` 目标解析+消息 ID 锚+receive 网关,见 §3.13)。
- v0.8.0(测试版,2026-09-01):三期 **M3 表达与修正**(`qzone_post` 说说发布/表达润色层(content 模型直写+人设口吻润色)/睡前日记旁路生成+API 直发+醒来延迟回注/self 消息回注/发布主动触发+种子自举/空间见闻当日摘要/点赞通知源C/通知独立于窗口/view_friend_feeds+inspect_image hash/发现层翻页,见 §3.13)。
- v0.8.1(测试版,2026-09-02):联调改进集(日记模板 v5 仿 diary_plugin 蓝本重排/`qzone_like`+`qzone_comment`+`qzone_post` 三工具全域可用+白名单只管非 qzone 工具/view_friend_feeds 返回格式可读性优化/通知消息不再强制触发回复概率,见 §3.13)。
- v0.8.2(测试版,2026-09-02):日记深化与输出卫生(日记模板 v6 照搬 diary_plugin 原文(像人的内心描写)/聊天时间线改 message.get_by_time 全天全局拉取/见闻+睡眠回顾 prompt 去机械化/全部可见内容截断尾加"...",见 §3.13)。
- v0.8.3(测试版,2026-09-02):配置固化与日志收敛(虚拟伪群号/显示名固化常量/源C 常规观测日志移除/润色模型默认 replyer/日记超长硬上限移除,见 §3.13)。
- v0.8.4(测试版,2026-09-02):截断清理与回执语义(回注/登记截断全删(用户逐项裁定)/润色层不截断/见闻叙事 100 字/-10049 频控回执/get_by_time list 兼容,见 §3.13)。
- v0.8.5(测试版,2026-09-02):通知死锁修复+润色事实保持+日程 RPC 超时(awaiting 卡死吞新评论的扫描死锁/润色模板 v6 不修改关键事实/schedule_llm_timeout_ms 默认 120000,见 §3.13)。
- v0.8.6(测试版,2026-09-02):终审修复波+裁定执行(登录态同轮自愈(#7)/bot 昵称直接抛错(#33)/同说说评论硬上限删除/通知发现-入队原子性回退/窗口收尾持泵锁/取消回队防丢在途项/压缩出事件循环+解码防护/失败图占位/mime 魔数探测/单图下载防炸/speak_threshold_level 校验,见 §3.13)。
- v0.9.0(测试版,2026-09-02):**说说详情工具与 reply 全域化**(view_friend_feed_detail 完整评论查看(楼中楼展开/Q9 上限/查看即已见)/qzone_reply 全域+comment_map 三级解析/view_friend_feeds 页码翻页/浏览注入带评论区(零新增 API)/删 recent_comments 死字段与 routing.py 死代码,见 §3.13)。
- v0.9.1(测试版,2026-09-03):**发现层协议校准**(scope=0→2 好友动态流(旧取值实为 2 天小窗口且只见自己动态)/begin 偏移→begintime 游标翻页(旧参数被服务端无视)/解析 appid 回退 appiconid 防窗口边界丢条,见 §3.13)。
- v0.9.2(测试版,2026-09-03):终审修复波(reply 通知上下文仅锚匹配采用(错靶修复)/评论区截断标签按显示数/空评论区诚实提示/comment_map 同键昵称保留,见 §3.13)。
- v0.9.3(测试版,2026-09-03):第三次全量复审修复波+图片链拼接化(昵称抛错点安全区(发布前读/补注种子入 try)/normalize_ts 与衰减防重入/日记发布同轮自愈/mark_seen 三态保锚/源C 时间折算防护+调用隔离/**图片公共管线:多图拼接合成(序号角标)替代逐张入管线,VLM O(N)→O(1),≤3 截断删除**/锚 hash 按拟合后字节/owner_nickname 语义修正,见 §3.13),即当前公测版本。
...
| 文件 | 职责(一句话) |
|---|---|
| `plugin.py` | 薄接线层:插件生命周期、8 个 Hook、11 个工具、1 个命令、11 个后台调度任务、全部 SDK 适配与日志(业务逻辑不在此)。 |
...
| `catsitate_core/llm_provider.py` | 旁路 LLM 统一出口:12 个内置 prompt 模板、主程序 prompt 管理覆盖加载、稳定段前置组装、模板版本化缓存键。 |
| `catsitate_core/qzone/` | QQ空间模块包(感知+互动+统一通知,工具驱动 v0.7):协议纯函数(g_tk 签名/callback 解析/说说与评论解析)、HTTP 客户端与 cookie 管理、动态去重存储(`qzone_feeds`)、评论去重与好感度事件存储(`qzone_comments`/`qzone_fav_events`)、赞事件去重(`qzone_likes`,源C)、注入消息构造(带工具 ID 锚)、串行注入状态机、注入上下文登记表(`registry.py`,工具目标解析)、表达润色层(`expression.py`,content 草稿按人设口吻润色)与场景纯函数(网关注册/启动自检/拉取与统一通知调度/发布 API `do_publish`/五工具接线在 `plugin.py`,见 §3.13)。 |
...
- **校验规则**(与工具修改共用):恰好 1 睡眠窗口、活动 1~8、窗口不重叠、睡眠时长在 [min,max]、kind ∈ {greeting, daily}、时间精确到分钟(带秒容忍解析后归一化)、`read_qzone`/`send_qzone` 标记仅 daily 窗口合法(非 daily 窗口的空间标记校验拒绝,钳制修复时清除,见 §3.13;旧 `qzone` 单键读取兼容,视为只读)。
...
- 内容 = 当前活动 + 下一活动预览 + 该窗口到期的备忘提醒:`[日程] 午后:发呆看雨(至16:00);接下来:买菜;备忘:周四交作业`(空白时间显示「自由时间」;窗口已触发过附加「(该窗口已过)」;`read_qzone` 窗口行末附加「(正在刷QQ空间)」明文状态——planner 据此知道此刻的「刷手机」是在刷空间;当天到期备忘取当前说话人/当前流相关,最多 3 条)。
...
### 3.9.3 内容护栏(`guard.py`,v1.0.0,默认关)

- **定位**:出站内容的**最后一道防线**——可配置正则**硬拦在 LLM 判定之外**:不依赖模型自觉,对已知违规模式在发布前机械拦截(存在理由见 §1.2「内容护栏兜底」;配置见 §4.13 `guard` 节)。`guard.enabled` 默认关,关闭时零编译零匹配零行为变化;开启后**全部会话生效**(内容级护栏,不区分流/平台、真实聊天与空间虚拟流同拦)。
- **匹配语义**:`re.search`(部分命中即中)、**大小写敏感**、无其它 flags,命中取首个命中规则的 1 基编号。`guard.patterns` 任一条非法正则在插件加载时**整组拒绝加载**并告警(错误串含坏规则序号/原文/异常类型)——护栏失效(不拦截任何内容)但不阻断插件加载,修正配置后重载恢复(错误显式暴露,不做部分可用兜底)。
- **三个拦截点**(命中即拦截取消发布,Q9=A):
  1. **空间三工具**:`qzone_comment`/`qzone_reply`/`qzone_post` 的**润色后最终文本**(润色失败草稿直发形态同样覆盖)发出前匹配,命中即拦截——零 API 调用、零记账、零 seen/回注登记;工具回执告知模型「内容被拦截(命中规则N),未发布」,模型可改写后重试(重试文本再过护栏)。
  2. **日记**:入睡任务 LLM 产出文本**发布前**匹配,命中即告警跳过——不发布、不落回注快照(没发出去的日记不该回注成「昨晚发过」的假上下文),当夜无日记。
  3. **replyer 回复**:钩子 `catsitate_content_guard` 挂 `maisaka.replyer.after_response`(BLOCKING **EARLY**,先于哨兵 LATE),对 response 文本匹配,命中即告警并**改写为空串**——主程序 reply 工具拿空文本走失败回执,planner 看到生成失败即本轮真沉默(空 response 同时使哨兵层入口短路,零 LLM 调用)。
- **拦截可重复生效**:拦截不是「一次否决后放行」——模型每轮新生成的回复/工具内容都会**再过一次护栏**(replyer 钩子按次执行、工具按调用执行),重调重发不存在漏发路径。
- **病态正则提示**:避免 `.*` 等**恒真**正则——会拦下一切回复/评论/日记(bot 彻底沉默),且**每次命中都打一条告警**;护栏正则应针对具体违规模式(敏感词/号码段等)编写,上线前建议离线验证。
...
- `@Tool("inspect_image")`:参数目标消息 ID(可选)或 `image_index`(倒数第几张含图消息,默认 1)+ 具体问题;另可选 `image_hash`(v0.8.0 起)——图片 hash 前缀(8 位即可,来源 `view_friend_feeds` 返回的图标注),给定时**跳过消息搜索**,直接按前缀查 Images 表,覆盖非消息来源(view_friend_feeds 等经 tool result media 入库)的图片。
...
- **浏览窗口内刷好友动态(M1)**:日程生成模板 v5 支持为 daily 窗口标记 `read_qzone`(刷空间)与 `send_qzone`(发说说)两个独立属性(通常一天 1~2 个 read 窗口、0~2 个 send 窗口,适合搭配轻松的独处活动如「窝着刷手机」;旧 `qzone` 单键读取兼容视为只读);`qzone_poll` 调度(默认 15 分钟)在 read 窗口内拉取好友说说——**拉取架构(统一时间线;协议 2026-09-03 实机校准为 scope=2 好友动态流+begintime 游标翻页)**:**发现层** `feeds3_html_more` 以 `scope=2`(全好友动态流,7 天窗口)拉取(请求 `discovery_count` 为条数上限,**实际条数由服务端窗口决定,不受控**);翻页为**游标推进**——续页唯一必需参数 `begintime`=上页响应 `main.begintime`(旧 begin 条目偏移已被服务端无视,删除),四重终止:空页/本页无新说说 tid(稳态首页全旧即止步恒 1 次调用)/游标耗尽/`discovery_max_pages` 上限(默认 3 页)——长时间离线后的动态积压逐页回溯不静默丢失(游标链实测 22 条/约两个月);解析对缺 `appid` 的窗口边界条目回退 `appiconid`,防整条丢失,**充实层**仅对发现层标记为新的 tid 按作者 uin 分组拉 `emotion_cgi_msglist_v6` 完整实体(典型 0-3 次/周期)——API 量从 O(好友数) 降为 O(1+新动态数),好友 24 或 240 成本相同;发现层非登录态失败告警回退旧逐好友路径(好友列表走 adapter OneBot API `adapter.napcat.account.get_friend_list`,remark 优先作昵称,逐好友拉最近 3 条,好友间固定 2 秒间隔防风控)。窗口切换时收泵,未注入的队列**回退未读**(queued 行删除,下个窗口重新可见,不丢动态);零新动态的轮同样算完成浏览——同窗形态的发布触发照常收尾派发(见下),不因无新动态而错过。
...
- **四个空间工具(v0.7 互动 + v0.8 表达;v0.8.1 三工具全域化)**:`qzone_comment(feed_id, content, at_user_id?)` 评论说说;`qzone_reply(feed_id, comment_id, content)` 楼中楼回复——**commentId+commentUin 二元组精确匹配主评论**(源A=好友评论/源B=bot 自己被回复的评论,@ 目标与二元组解耦由 wire 层承载);`qzone_like(feed_id?)` 点赞(缺省对当前浏览的说说,通知项自动锚定其原说说);`qzone_post(content)` 发布自己的说说(无频控——是否发/表达什么由模型自主决定)。`content` 由模型直写草稿、发出前经表达润色层顺口吻(见下条)。**`qzone_like`/`qzone_comment`/`qzone_post` 全域可用(v0.8.1,2026-09-02)**——`view_friend_feeds` 提供说说ID/图片hash 后,真实聊天流里同样可以点赞/评论/发说说;v0.9.0 起 `qzone_reply` 亦全域可用(comment_id 锚三来源:通知参数行/浏览评论区/view_friend_feed_detail;无锚显式拒绝+指引)。动作 API 业务错误不重试(回执带限制语义);**登录态失效同轮自愈(v0.8.6,用户裁定 #7:不得放弃本轮动作)**——作废并强制重取 cookie(NapCat 在线会话免扫码)后原地重试一次,重取失败/仍失效才显式失败。
- **全域查看工具 `view_friend_feeds(qq, count?)`(v0.8.0;v0.8.1 格式可读性优化)**:拉指定好友最近说说(每页默认 3 条上限 10,`page` 页码翻页——Q8=2026-09-02 设计共识;翻到底诚实提示「没有更多了」;**不含评论区**,看单条完整讨论用 view_friend_feed_detail),**任何聊天流都可用**——也是 `qzone_like`/`qzone_comment` 等空间动作工具在真实聊天流里的参数来源(结果带说说ID);回 dict——`content` 文本摘要(**可读性格式:头部「{昵称}(QQ:{qq})最近的说说,共 N 条:」同主点名一次,条目编号〔1〕〔2〕…+发布时间前缀(浏览注入同款相对时间)+正文(超 300 字截断,尾加"...")**,图片走与浏览注入同一条公共拼接管线(多图合成一张带序号角标,单图直发;锚=**实际送出字节**的 sha256 前 8——多图「图1-图N(拼接,hash=…)」单条,部分下载失败时单图锚用幸存图的原始序号;`content_items` 恒 ≤1 项,base64 经 tool result media 入 Images 表供 `inspect_image` 反查;超预算压缩重编码后锚按拟合后字节重算,压不动丢弃则锚清空);成功即 registry 登记(content_summary/recent_comments=说说正文与近评摘要,上下文素材);登录态失效与动作工具同款同轮自愈(重取后原地重试)。
- **全域详情工具 `view_friend_feed_detail(feed_id, qq?)`(v0.9.0,设计共识 Q1A/Q2B/Q4/Q5B/Q10B/Q12A)**:查看**单条说说的完整信息**——正文全文+图片(公共拼接管线,同 view_friend_feeds 契约)+**全部顶层评论与楼中楼**(每条评论楼中楼最多展开 10 条+「共N条回复」标注,整块超 6000 字截断标注「评论过多,只显示前面部分」——Q9 上限;QQ 侧截断 commentlist 时头部标「前N条/共M条」)。`feed_id` 必填(说说ID锚),`qq` 选填(缺省经 registry/seen 三级解析主人,解析不到显式索要——Q12)。**副作用**:查看即 `mark_seen`(Q10=B:浏览轮询不再把该说说当新动态注入;源A 评论扫描不受影响);registry 登记 `comment_map`(评论级锚,qzone_reply 目标解析用)。说说不在最近 20 条内时诚实提示(更早的说说无查看通路,已知限制)。
- **评论级锚 comment_map(Q6)**:`FeedContext` 新增 comment_tid→(主评论作者 uin,昵称) 映射——浏览注入(结构化评论全量)与详情查看(全量)通知注入(主评论二元组)三路填充,键级合并(新评论并入,旧评论锚保留);旧「近评摘要」字符串字段已删(Q7:无消费方死字段)。
- **qzone_reply 全域化(v0.9.0,Q3/Q11)**:移除虚拟流门控,任何聊天流可用;comment_id 锚来源=空间流通知参数行、浏览注入评论区、或 view_friend_feed_detail 结果。目标解析三级(v0.9.2 修正:通知上下文**仅在锚匹配**时采用——registry 合并会让通知登记过的说说保留旧二元组,无条件优先会把 comment_map 命中的另一条评论错挂线程):①通知上下文且 comment_tid==comment_id ②comment_map(浏览/详情登记)③全 miss 显式拒绝+指引「先用 view_friend_feed_detail 查看该说说」——不猜测回退,防 @ 错人。
- **浏览注入带评论区(v0.9.02 设计共识)**:注入消息正文后拼接评论区(空行分隔)——顶层评论每行「昵称(QQ):内容 (相对时间)〔评论ID=…〕」,楼中楼缩进「  ↳ …」一行一条(上限同 Q9),锚照抄契约供 qzone_reply;数据来自充实层现有 msglist 载荷(list_3 楼中楼在响应内,**零新增 API 调用**)。
- **表达润色层(M3 修正,润色架构)**:`content` 由模型直写草稿——「说什么」归模型(它持有全量上下文与表达意图),「怎么说」归润色层:旁路人设 LLM(`qzone_expression` 模板,`expression_llm_model`,v0.8.2 起默认 `replyer` 与主程序回复模型同源)按 bot 人设(主程序 `personality.personality`)与表达方式(`personality.reply_style`)把草稿顺成 bot 平时的样子,与主程序 planner/replyer 分工同构;两段配置均经 config.get 读取带缓存,bot 配置热更新自动失效。护栏:评论/回复正文上限 200 字、说说上限 500 字(入参即校验;同说说评论硬上限已删(v0.8.6 用户裁定),刷屏防护=QQ 侧频控+-10049 限制回执;润色超长带字数要求重润一次,重润仍超**不再截断**(2026-09-02 用户裁定)——按模型原样发出);模板 v6 起**不修改关键事实部分**(人名/数字/时间/地点/明确说过的话与做过的事保持原样,只调整说法和语气——完全重组许可曾会改掉事实);**润色失败不阻断动作——告警后以草稿直发**(草稿即模型的完整表达,显式回退不静默);@ 前缀拼装(napcat 同格式,registry 有该评论者昵称则用昵称)沿用不变。
- **日记(M3 表达,入睡任务旁路生成+API 直发+延迟回注)**:入睡时与次日日程生成同属入睡任务——用旁路 LLM(`qzone_diary` 模板,v0.8.2 升 v6 **照搬 diary_plugin prompts.py 原文**,仅占位符适配两段式布局:{target_length}→素材目标行/{date_with_weather}→素材括注/{style_desc}→内联蓝本默认值「日记风格(私人记录,带反思感想)。」;核心=像人的内心描写:日期天气开头/睡前随手写/反流水账要有感情色彩/简短口语可以有个性)以 **bot 本人身份**书写。素材为**蓝本形态**:「我的名字是{bot 昵称}\n{人设}\n今天是{年月日},回顾一下到现在为止的聊天记录:\n{时间线}」+当日其余素材(日程/备忘/见闻/天气/目标字数),尾行「日记内容:」作生成引导(单串蓝本里紧跟指令,两段式布局放素材尾等价)。每次生成随机目标字数 n∈[80,200] 注入「(目标 {n} 字左右)」。经发布 API(`do_publish`)直发为说说;旁路 LLM 与发布 API 均不经消息链,**不受睡眠拦截(深夜直发)**;睡眠窗口终点未入睡时由 tick 补执行(每窗口至多一篇)。**聊天时间线=当日全部聊天记录(v0.8.2 对齐蓝本 message_fetcher)**:经 `message.get_by_time` **全局**拉当日 00:00 起的全部消息(跨全部聊天流,limit=0 不限条数——旧逐流 get_recent 只覆盖插件缓存的流且限量,一天的记录拿不全),逐条「[HH:MM] 谁:内容」时间序铺开(bot 标「我:」、他人标昵称、单条截 100 字尾加"..."、总量超 300 条保留最近并标注「更早的聊天已略」);空间虚拟流消息按平台剔除;能力失败显式告警后回退旧逐流路径。真实天气=当前快照(无数据省略——模板口径「没给就只写日期」);素材行取数失败均告警后省略,不拖垮日记主链路。护栏:空文本跳过;**不设超长硬上限**(v0.8.2 对齐 diary_plugin,长度由模板目标字数软约束);发布失败不落回注快照(没发出去的日记不该回注成「昨晚发过」的假上下文)。**回注延迟到醒来**:正文+发布时刻存 `qzone_pending_diary.json`,醒态 `sleep_tick` 以 self 消息「我昨晚发布的日记:{全文}」补注进虚拟流(2026-09-02 用户裁定:回注/登记截断全删)(睡眠期注入会被 sleep_gate 拦进回顾缓冲,白注入);补注失败保留快照下轮重试。开关 `diary_enabled`(默认开)。
- **统一通知通道(M2.1,spec §3.7;M3-r2 扩三源)**:模拟推送通知——`qzone_notify_poll` 调度(默认 120 秒,`notification_interval_seconds`,注册时下限 30 秒;始终运行,醒着即可,与浏览窗口无关)**三源检测**:源A=好友在 bot 自己说说下的新评论(扫自己最近 `own_feed_scan_count` 默认 20 条说说——好友翻旧账评论也要能发现,单次请求条数不增加调用次数);源B=bot 在他人说说下的评论收到的新楼中楼回复(list_3,目标好友由 bot 评论留痕反查圈定);源C=好友赞了 bot 的说说(M3-r2:「与我相关」流 `feeds3_html_more?scope=1` 解析赞事件,拉最近 30 条;自己的赞跳过;去重走 `qzone_likes` 表,键=赞者+说说,同一人同一条说说只通知一次——取消赞再赞不重复;登录态失效作废 cookie 但不中断本轮,源A/B 已得通知不丢)。通知走**双优先级队列 P1**(插队于浏览动态 P2 之前),模拟「刷着动态→弹通知→先看通知→回完继续刷」注意力模型;**通知注入是推送语义(M3-r2 P1/P2 分治):任何时刻可注入(仅 awaiting 互斥),不依赖浏览窗口——浏览窗口结束只清浏览队列 P2,通知队列保留等待注入条件**,浏览动态仅 read_qzone 窗口内注入;单轮至多 3 条(防通知风暴),早于 `summary_days` 的过旧通知/赞事件截断不注入,阅读顺序新→旧(信息流降序,QQ 空间 App 实际形态)。通知正文自然可读并带参数独立尾行(可读性优化 2026-09-01)——源A「评论了你的说说:…\n〔说说ID=xx 评论ID=xx 评论者QQ=xx 评论于(今天HH:MM)〕」、源B「回复了你的评论「{bot原评论前20字}」:…\n〔说说ID=xx 评论ID=xx 评论者QQ=xx 回复于(今天HH:MM)〕」、源C「赞了你的说说「{自己说说摘要前20字}」\n〔说说ID=xx 点赞于(今天HH:MM)〕」(参数行末段为动作时间,`create_time` 缺失则省略不编造;源C 无评论ID/评论者QQ,标题摘要取发布回注登记的自己说说——未登记则无标题不臆造;楼中楼上下文=bot 被回复的原评论前 20 字,缺内容回退「你之前的评论」;评论内 `@{uin,nick}` 机器格式解析为「@昵称 」可读形态)。**通知不设 is_mentioned(v0.8.1,2026-09-02)**——走主程序自然回复概率,不强制触发 planner 轮:bot 看到通知但不必然回应(是否回应/何时回应交给自然概率与回复必要性判定,拟人化留白;浏览注入仍保留强制触发,串行浏览决策环的设计依赖);注入消息带 reply 段引用**原说说**的注入消息(napcat quote 式,引用内容=原说说正文前 60 字(截断尾加"...");原说说未注入过则省略 reply 段回退纯文本);上一条注入仍在等待轮完成(awaiting)时不取新通知(不叠加,下轮再取;v0.8.5 起扫描遇 awaiting 先驱动泵——窗口结束后 planner 长期不跑轮时 awaiting 超时兜底原先无人触发,「占用→跳过扫描」死锁会吞掉好友新评论,实机卡 18 分钟定位)。通知项的注入被宿主拒绝或异常时**回退去重键**(下轮通知轮询重新发现,不因一次拒绝永久丢失;深度审查 B-4),但**重试上限 3 次**(深度审查 A-N1:软回退保留行,计数跨重发现累计;满 3 次仍被拒则保留登记放弃——防宿主持续拒绝时每 120 秒无限重注入);同一通知事件的 `fav_events` 同日去重(重发现不重复入库)。
- **好感度显式事件(M2,spec §3.9)**:好友评论 bot 说说(源A)/好友赞 bot 说说(M3-r2 源C,kind=LIKE)/ bot 评论·回复·点赞好友,双向记入 `qzone_fav_events` 事件表——结算时按 `[空间互动]` 前缀并入该人日终结算素材(LLM 计权,事件按原始时刻去重防同日 early→daily 重判),并参与自然衰减计时基准;**不依赖 batch_counter**(虚拟流消息计数已被豁免)。
- **memo 按人语义联动(M2)**:虚拟流上写的备忘与真实聊天跨流可见(主 QQ+附带 QQ,见 §3.6.1)。
- **虚拟流注入(主链可引用, M1)**:每条动态构造为一条消息注入 `qzone-qq` 虚拟群聊流,复用主程序 planner→replyer 链;注入带 `is_mentioned=1.0` 强制触发;消息时间戳=阅读时刻(注入时刻,消息流时钟单调),发布时间以相对时间前缀写入正文(今天 HH:MM/M月d日 HH:MM,bot 不会把老说说当成刚发生;方案 B 2026-08-31);文本段末尾带「〔说说ID=xxx〕」参数独立尾行(v0.7 锚,v0.7.1 起换行独立成行,**纯图说说也保留文本段承载锚**);「刷到但懒得理」由 planner 自主沉默(无工具调用轮)表达——意愿判断权归模型。图片走**公共拼接管线**(v0.9.3 用户裁定 C 方案,实机实证 9 图→1 合成→1 次 VLM):单图原样直进;多图(≥2)拼接为一张 3 列网格合成图(格约 640px、白底 letterbox、左上角圆底**原始序号角标**——下载失败的图按原序号跳格不重排,空位即示缺),一张图进主流水线(描述/落 Images 表)——VLM 与上下文成本从 O(N) 降为 O(1),模型可对合成图经 `inspect_image` 问「图3是什么」;体积治理=合成后单图过压缩阶梯与 12MB 预算(极端丢弃保帧并告警),全部图片下载失败时正文占位 `[图片×N]`。
- **说说发布回注(qzone_post)**:发布成功的说说立即以 self 消息注入虚拟流(`qzone_self_` 前缀 message_id,user=bot 自己,**无 is_mentioned**——不触发 planner 决策轮,仅入历史)——后续好友评论该说说时,bot 需要这段历史才知道自己发过什么;**回注正文带全文**(2026-09-02 用户裁定,原 60 字预览);seen/registry 登记同样存全文;回注失败不影响发布回执(说说已远端发布,谎报失败会诱导重复发布)。新说说 tid 三处登记(回注锚/registry/seen 表,owner=bot 自己)——「发布→好友评论→回应」链路不断。
- **发布主动触发(M3-r2,send_qzone 窗口)**:日程 `send_qzone` 标记的 daily 窗口触发分享指示,按窗口形态分两种——**同窗形态(read+send)**:首轮浏览拉取收尾、泵空闲后派发(等待上限 180 秒,超时也触发——刚看完好友动态,分享有上下文;零新动态轮同样算完成浏览,照常派发);**仅 send 形态**:窗口开始即派发(无浏览语义,「忙里偷闲发一条」)。指示经 `maisaka.proactive.trigger` 注入虚拟流(intent 说明可用 `qzone_post` 直接写想发的内容;不想发保持沉默)——是否发布、表达什么由模型自主决定,触发不保证发布。**冷启动种子自举**:虚拟流会话在首条消息进入后才诞生,触发报「未找到聊天流」时先注入一条无 is_mentioned 的 self 种子消息(「(打开了QQ空间)」,仅建会话不触发决策轮)再带间隔重试数次(会话注册是异步的,立即重试会早于会话建立);仍失败告警等下个窗口。窗口判重按「{day}|{窗口start}」标记(邻接 qzone 窗口间泵状态不复位,不能当切换信号);read→仅 send 邻接切换时浏览队列同样回退未读(send 窗口无浏览语义)。日志:`QQ空间发布触发已发出(browsed=…)`、`QQ空间发布触发未成功(…),等下个窗口`。
- **真实聊天见闻注入(M3-r2 见闻系统优先,近 24h 滚动印象)**:read_qzone 窗口结束时(含 read→仅 send 邻接切换)旁路 LLM(`qzone_digest` 模板,`digest_llm_model`)把**近 24h 滚动窗素材**(浏览 `recent_seen(days=1)` 最新 15 条动态叙事+互动 `fav_events_window(now-24h)` 最新 10 条互动文本,同窗锚点、截断统一保留最新)摘要为 60~150 字「空间见闻」,存 `qzone_digest.json`(键 date/text,date=生成当日);见闻语义为「近 24h 滚动印象」而非自然日印象(2026-09-04 翻案 H-2 自然日旧裁定)——跨零点窗口结束时昨晚的浏览与互动素材自然衔接;真实聊天流的注入块当日优先输出 `[空间见闻] {text}`(注入键 `qzone:d:{date}:{长度}`,见闻更新即换键),**无当日见闻回退既有「近期刷到」路径**。生成走插件侧窗口边界旁路摘要——虚拟流 receive-only 无发言投递,主程序记忆摘要层不会为它产出内容,插件亦无 API 读取记忆段落(素材→摘要→存储→注入,与主程序记忆摘要方法一致)。护栏:窗内无素材不生成(保留旧见闻);LLM 失败/输出异常(空或超 400 字)告警并保留上一份。开关 `digest_enabled`(默认开)。
- **真实聊天见闻摘要注入(M3 表达叙事格式,回退路径)**:真实聊天流的注入块附带 `[空间] 近期刷到: 昵称发了「摘要」;…`(近 `summary_days` 天已 seen 的 `summary_count` 条动态,叙事格式与浏览动态的自然文本一致;摘要截 100 字(2026-09-02 用户裁定,原 20),纯图说说以「图片」占位,缺昵称回退QQ号)——bot 在真实聊天中可自然引用「我看到你发的说说」类见闻;当日无「空间见闻」(digest)时启用本路径。
...
- 虚拟平台名常量 `qzone-qq`;伪群号/显示名为**不可配置常量**(v0.8.2 用户裁定:`qzone_feed`/「QQ空间」,固化于 `catsitate_core.qzone`)——可配置会被改成与真实群号相同的值,会话路由与 person 折叠随之漂移;旧配置残留键无效可移除。
...
- **场景替换**:planner(`before_request`)与 replyer(`before_model_request`)两侧,把 system 文本中的群聊场景提示词(主程序 `chat.reply_style.group_chat_prompt` 当前值,1 小时缓存)**原位精确替换**为空间场景文案(按配置值匹配,用户改过配置也能命中;说明〔〕参数行格式、四工具用法(含 qzone_post 发说说)、润色架构参数(评论/回复/发布的 `content` 由模型直写,发出前自动按口吻顺一遍)与点赞通知(「赞了你」也是互动通知),并明示「直接打字是发不出去的」;v5 与润色架构工具参数形态逐字对齐)。**场景文案可配置(2026-09-01)**:模板 `catsitate_qzone_scene` 走三层链——主程序 `data/custom_prompts/zh-CN/`(WebUI 编辑产物,优先)→ `prompts/zh-CN/`(插件 on_load 自动部署,prompt_templates 为权威源)→ 插件内置;WebUI 改完即生效(mtime 缓存失效自动重读)。虚拟流注入块已去重只留动态状态(场景全文只在 system 段出现一次);配置为空 / 未命中(主程序模板改版风险)→ 每类每进程告警一次,本轮无场景说明(工具链仍可用)。
- **工具白名单与全域工具(v0.8.1 重定义,2026-09-02)**:`qzone_*` 前缀工具(`qzone_like`/`qzone_comment`/`qzone_reply`/`qzone_post`)**全域默认可用、不受 `tool_whitelist` 管理、不可剔除**——`view_friend_feeds` 提供说说ID/图片hash 后任何聊天流都能互动,白名单里写不写都不影响(写了只会在 on_load 收到「该项无效可移除」告警)。白名单只管其余工具在**虚拟流**的可见性,默认 `wait/query_memory/query_person_profile/memo_write/memo_read/inspect_image/view_friend_feeds/view_friend_feed_detail`(**不含 tool_search/msg_react/poke_user;`reply` 已废弃**——receive 网关无出站路径);真实聊天流不经此过滤(主程序自选,qzone_* 与其他工具原样放行)。**旧配置兼容**:持久化白名单缺 `view_friend_feeds` 时 on_load 告警提示补入(虚拟流内将无法查看好友说说);残留 `qzone_*`/`reply` 项一并提示可移除(不静默改配置)。
...
- `QQ空间动态注入被宿主拒绝(tid=…,adapter policy 或网关状态),跳过且不标记已见`;`QQ空间通知被拒,回退去重键待下轮重试(第 N 次)`(深度审查 B-4+A-N1:通知项注入失败软回退 is_new 登记键,计数跨重发现累计;源A/B 评论通知适用);`QQ空间通知重试 3 次仍被拒(dedup_key=…),放弃不再重试`(A-N1 上限:保留登记不再重注入);`源C 赞事件通知被拒,放弃(源C 无重试通道)`(源C 去重在 `qzone_likes` 表,无软回退通道——注入被拒即放弃,不重试也不误报「待下轮重试」)
...
- `QQ空间发布触发已发出(browsed=…)`(M3-r2 send_qzone 窗口分享指示成功,browsed=同窗形态/仅 send 形态;含种子自举重试成功形态);`QQ空间发布触发未成功(…),等下个窗口`(触发失败含种子自举后仍失败);`QQ空间发布触发跳过:虚拟流 session id 不可得`;`QQ空间种子消息注入失败`(冷启动自举种子注入异常)
- `QQ空间表达润色失败,以草稿直发` / `QQ空间表达润色返回空文本,以草稿直发` / `QQ空间表达润色调用异常(RPC 超时(E_TIMEOUT: …),以草稿直发`(润色层失败回退:草稿即模型完整表达,显式告警不阻断;v0.8.5 起调用异常同样兜底,RPC 超时以「RPC 超时」明显浮出);`QQ空间表达润色超长(N 字>上限 …),这次改短一些重新润色`(软性改短重润,重润结果即最终文本;v0.8.4 起不再截断)
- `QQ空间日记发布成功: …`(入睡任务日记直发);`QQ空间日记 LLM 生成失败,跳过本轮` / `QQ空间日记 LLM 失败(success=…),跳过`(旁路生成失败,当夜无日记);`QQ空间日记内容为空,跳过发布`(空文本护栏;v0.8.2 起不再设超长硬上限,长度完全由模板目标字数软约束,对齐 diary_plugin);`QQ空间日记发布失败(内容已生成,发布跳过)`;`日记聊天素材取数失败(stream=…),该流跳过` / `日记天气素材读取失败,素材行省略`(v0.8.0 日记素材可省略行:单素材失败告警后跳过,不拖垮日记);`QQ空间日记醒来补注完成`;`QQ空间日记补注失败(下个 tick 重试)`(快照保留,醒态 tick 重试)
...
- `QQ空间通知入队 N 条(源A+B+C,P1 插队)`(M2.1 统一通知:三源检测到新评论/楼中楼回复/赞入 P1 队列);`QQ空间通知轮询源A失败,本轮跳过`;`QQ空间通知轮询源B失败(好友 …),该好友跳过`;`QQ空间通知轮询源B好友反查失败,本轮跳过源B`;`QQ空间通知轮询源C失败,本轮跳过源C`(M3-r2 源C:赞事件拉取失败,源A/B 已得通知不受影响);`源C 连续 3 轮解析 0 条赞事件(模块可用),解析锚点可能漂移…`(warn-once:取数成功但持续零事件,提示 HTML 锚点漂移需实机核对,恢复有事件后复位);`QQ空间赞事件过旧跳过(…)`(源C 早于 `summary_days` 截断;当前响应无时间锚,该截断暂不生效,见已知限制);`QQ空间赞事件好感度记账失败(仅告警)`;`QQ空间评论过旧跳过(…)` / `QQ空间楼中楼回复过旧跳过(…)`(早于 `summary_days` 截断);`QQ空间…遇登录态失效,cookie 已重取,本轮原地重试`/`cookie 重取失败(adapter 无有效登录态)`(同轮自愈,v0.8.6);`QQ空间动态注入被宿主拒绝`/`QQ空间动态已注入`(通知注入与浏览注入同走串行泵日志)
- `QQ空间点赞遇登录态失效,cookie 已作废,下轮重取`(v0.7 `qzone_like` 自愈链)
- `QQ空间数据清理:评论去重 N 行,赞去重 M 行,seen 保留 7 天`(深度审查 D-1:每日 prune 任务,qzone_comments/qzone_likes 30 天+qzone_feeds seen 行 7 天)
...
1. **日记生成频率与素材边界**:日记属入睡任务,每睡眠窗口至多一篇(入睡或窗口终点补执行,二者只走其一);素材只取当日真实数据——日程/备忘/空间见闻三源之外,v0.8.0 补聊天时间线(真实聊天流当日消息按小时分段,虚拟流消息不含)与真实天气(当前快照,无数据省略),可省略行失败告警后跳过不拖垮日记;内容保真由模板「不得编造」约束+模型决定;LLM 失败当夜无日记,不补生成。
...
6. **写路径有真实副作用**:评论/回复/点赞/发布说说/日记一经工具或入睡任务成功即真实发布(无撤回通路);动作 API 失败不重试。`qzone_post` 无频控(正文生成上限 500 字护栏之外全凭模型自主),日记每窗口至多一篇。好友对 bot 说说点赞经源C 通知并计好感度事件(M3-r2 起覆盖);bot 主动点赞经工具路径计入。
...
8. **源C 赞事件当前无时间锚**:「与我相关」流(scope=1)响应未携带事件时间——新鲜度截断不生效(`summary_days` 对源C 无效)、通知参数行「点赞于」时间段省略(不编造时间);上线首轮的存量赞按单轮 3 条上限分轮消化属预期,非缺陷(实机验收对照)。
...
- **`schedule_generate` 模板升 v4 后,WebUI 自定义过的该模板需手动同步**:插件自动部署只覆盖主程序 `prompts/zh-CN/` 内置层,`data/custom_prompts/` 下的 WebUI 编辑产物优先级更高且不会被覆盖——旧版自定义模板不含 `read_qzone`/`send_qzone` 属性说明,日程将不产生空间浏览/发布窗口(需在 WebUI 手动更新或删除该自定义模板)。**`qzone_scene` 模板同理(v0.8 升 v5)**:WebUI 自定义过的该模板需手动同步,否则场景说明不含润色架构参数(`content` 模型直写、自动顺口吻)与点赞通知用法(模型不知道发出的内容会润色、赞了你也可回应)。
- **写路径有真实副作用(风险提示)**:评论/回复/点赞/发布说说一经 `qzone_comment`/`qzone_reply`/`qzone_like`/`qzone_post` 调用成功即真实发布到QQ空间(无撤回通路);**是否互动、表达什么方向、是否发说说,完全由模型自主决定**(content 由模型直写、发出前按人设润色;插件只做工程护栏——正文上限 200/500 字(入参校验;润色重润仍超不截断,v0.8.4;同说说评论硬上限已于 v0.8.6 删除——防护交 QQ 侧频控+-10049 限制回执)、`qzone_reply` 虚拟流会话门控、目标三级解析失败显式拒绝、润色失败以草稿直发不静默);**三动作工具全域可用(v0.8.1)**——真实聊天流里模型同样可能调它们点赞/评论/发说说,关闭 `qzone.enabled` 才是整体停用开关。入睡任务还会每晚自动发布一篇日记(`diary_enabled` 可单独关闭)。`qzone.enabled` 关闭即整体停用(读+写一起);不希望写动作时关闭该开关或停用模块,不存在"只读不禁写"的中间档。
...
- **旧配置升级(v0.7→v0.8.1)**:持久化的 `qzone.tool_whitelist` 若缺 `view_friend_feeds`,on_load 告警提示补入(虚拟流内将无法查看好友说说);`qzone_*` 项与已废弃的 `reply` 项在 v0.8.1 起不再被白名单消费——告警提示可移除,不移除也不影响功能(qzone_* 工具全域默认可用)。
...
| schedule_llm_timeout_ms | 120000 | 日程生成 RPC 超时(毫秒)。此值是插件→主程序 RPC 请求死线,不设(0/None)时 RPC 层固定 30s——memory 模型跑日程大 prompt 实测超 30s,每晚 RPCError(E_TIMEOUT) 走模板兜底(2026-09-02 定位),故默认 120s |
...
| discovery_count | 50 | 发现层单页请求条数上限(实际条数由服务端窗口决定,不受控——2026-09-03 实证) |
| discovery_max_pages | 3 | 发现层游标翻页上限(begintime 游标推进;长时间离线后的积压回溯;稳态首页全旧即止步恒 1 次调用) |
| own_feed_scan_count | 20 | 通知源A扫描自己最近 N 条说说的评论(单次请求条数,不增加调用次数) |
...
| 字段 | 默认 | 说明 |
|---|---|---|
| enabled | **false** | 内容护栏总开关(默认关;关闭时零编译零匹配,一切行为零变化,见 §3.9.3) |
| patterns | [] | 拦截正则列表:**re.search 语义、大小写敏感**(部分命中即中),命中即拦截取消发布(三拦截点见 §3.9.3);任一条**非法正则将使整组护栏拒绝加载**并告警(错误串含坏规则序号/原文/异常类型;护栏失效不拦截任何内容,但不阻断插件加载,修正后重载恢复);**勿配置 `.*` 等恒真正则**——会拦下一切回复/评论/日记且每次命中都告警 |
...
| qzone_likes | like_key(PK), liker_uin, target_tid, created_at | 赞事件去重(M3-r2 通知源C):同一人同一条说说只通知一次(取消赞再赞不重复);表保留 30 天(每日清理任务,与 qzone_comments 同轮) |
| qzone_fav_events | id, day, user_id, kind, text, created_at | M2 好感度显式事件(spec §3.9):kind=COMMENT(好友评论 bot)/LIKE(好友赞 bot,M3-r2 源C)/OUT_COMMENT(bot 评论好友)/OUT_LIKE(bot 点赞);并入日终结算素材与衰减计时（无清理,只增不减——刻意保留作衰减计时基准）;**同日同 user+kind+text 去重**(A-N1:通知回退重发现不重复入库);当日事件 user 并入日终结算候选(C-N1) |
...
| qzone_digest.json | `{"date": 当天, "text": 空间见闻}` | 空间见闻摘要(素材近 24h 滚动窗,当日快照;read_qzone 窗口结束旁路生成;次日覆盖,注入真实聊天的 `[空间见闻]` 块) |
...
| qzone_poll | max(poll_interval_minutes,1)×60 s(默认 15 分钟;统一时间线架构下即**发现层间隔**——发现层 scope=2 好友动态流+begintime 游标翻页(上限 `discovery_max_pages` 页,稳态恒 1 次调用),仅对发现的新动态按好友充实,量与好友总数无关) | 空间窗口(kind=daily 且 read_qzone 或 send_qzone)内:read 窗口走发现层统一时间线→新动态按作者分组充实→去重入队→串行注入 `qzone-qq` 虚拟流(发现层失败告警回退旧逐好友路径);send_qzone 窗口派发发布主动触发(仅 send 即派,同窗形态首轮浏览收尾后派,含零新动态轮);read 窗口结束收泵回退未读+评论频控计数重置(v0.7)+见闻生成;睡眠期/模块停用跳过;长 IO(发现/充实 HTTP)在后台任务执行,不阻塞调度器 tick(深度审查 A-2,防重入标记跳过重叠轮) | 「QQ空间窗口开始,注入泵激活」「QQ空间新动态入队 {n} 条」「QQ空间动态已注入(tid=…,作者=…)」「QQ空间统一时间线拉取失败,回退逐好友旧路径」「QQ空间浏览窗口结束,浏览队列回退未读({n} 条);通知队列保留等待注入」「QQ空间发布触发已发出(browsed=…)」 |
...
4. **旁路模板自动部署**(无需手动):插件加载时自动把 `prompt_templates/catsitate_*.prompt`(12 个)同步到主程序 `prompts/zh-CN/`(内容一致跳过、变更覆盖;主程序 `load_prompts()` 在插件启动后调用,同次启动即生效,无需重启)。此后 WebUI「提示词管理」页显示并可编辑这 12 个模板(编辑产物写 `data/custom_prompts/zh-CN/`,插件优先读取);插件不在 `plugins/` 下或主程序 `prompts/zh-CN/` 目录缺失时跳过并告警(日志「旁路模板自动部署跳过」),插件回退内置默认,功能不受影响。
...
| 内容护栏 | `内容护栏拦截:回复 命中规则{n},置空未发送(文本:…)`;`内容护栏拦截:qzone_comment/qzone_reply/qzone_post 命中规则{n},未发布(文本:…)`;`内容护栏拦截:日记 命中规则{n},未发布(文本:…)`;配置错误告警:`内容护栏正则编译失败,整组护栏失效(不拦截任何内容,请修正配置):…` |
| 旁路记账 | `旁路 LLM 当日调用次数已达或超过阈值 {n},请注意用量`;llm_usage 表按模块分列 |
| quote 解析 | `quote 发送者解析: 成功 {n}/{m}(stream=…)`;`quote 发送者解析失败(stream=…):…` |
| QQ空间 | `QQ空间窗口开始,注入泵激活`;`QQ空间新动态入队 {n} 条`;`QQ空间动态已注入(tid=…,作者=…)`;源C 告警(v0.8.2 起常规轮次不打解析条数日志,仅异常信号):`源C 连续 3 轮解析 0 条赞事件…解析锚点可能漂移`(warn-once)/`源C 赞事件通知被拒,放弃(源C 无重试通道)`;见闻:`QQ空间见闻已生成({n} 字)`;v0.7 工具:`QQ空间评论失败(feed_id=…)`、`QQ空间楼中楼回复失败(feed=…,comment=…)`、`QQ空间点赞失败(tid=…)`;告警/停用:`QQ空间模块停用:…`、`QQ空间模块停用:person 别名折叠自检失败(…)`、`QQ空间网关收到意外出站回调(…)`(receive 网关防御分支)、`QQ空间场景回退:…`、`QQ空间说说拉取失败(uin=…),该好友本轮跳过` |
...
| QQ空间没有动态注入 | 查启动日志有无「QQ空间模块停用」(focus_mode 开启 / talk_value=0 / bot_user_id 为空 / person 自检失败)与「QQ空间虚拟平台就绪」;确认日程有 read_qzone 标记的 daily 窗口且当前处于窗口内(旧 `qzone` 单键兼容);「QQ空间说说拉取失败(uin=…)」→ NapCat 可否响应 `adapter.napcat.account.get_cookies`;msglist 条目即说说(转发走回退链/纯图以图段承载/视频占位);WebUI 自定义过 schedule_generate 旧模板需手动同步(§3.13.8) |
| 空间评论/回复/点赞没发出 | 工具返回「未找到说说 …」= 目标三级解析全miss(registry 过期+seen 超窗+非当前浏览项),让 bot 核对消息尾部〔〕参数行里的说说ID 或等下轮注入;「这条说说你已经评论过 3 次了」= 同说说频控上限(窗口边界重置);「QQ空间评论失败/楼中楼回复失败/点赞失败」→ 多为登录态失效(看有无「cookie 已作废,下轮重取」,自愈不重试);虚拟流里 bot 打字发不出去属设计(receive 网关,动作只能经工具);统一通知不出 → 确认 comment_poll_enabled 开启、bot 醒着(睡眠期静默)、上一条通知未被 awaiting 占用(不叠加)、bot 有历史说说且有人评论(源B还需 bot 曾评论过好友说说;源C点赞通知需有人赞过 bot 的说说且发布回注/已读表有该说说);说说发布主动触发不出 → 确认日程有 send_qzone 标记窗口,查「QQ空间发布触发未成功(…),等下个窗口」(冷启动种子自举失败会告警) |
| 源C 点赞通知无「点赞于」时间或首轮扎堆/迟迟不出现 | 源C「与我相关」流当前无时间锚:新鲜度截断不生效、「点赞于」时间段省略(不编造时间)、上线首轮的存量赞按单轮 3 条上限分轮消化——均属预期非缺陷;持续零事件看有无「源C 连续 3 轮解析 0 条…锚点可能漂移」告警(有则需实机核对响应形态;v0.8.2 起常规轮次不打解析条数日志) |
