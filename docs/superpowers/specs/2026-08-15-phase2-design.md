...
3. **睡眠即日程窗口,日程按活动排列(动态数量)**:全天日程 = **恰好 1 个睡眠窗口 + 1~8 个活动窗口**,活动窗口数量与名称由 LLM 生成时自主决定,也可经 `update_schedule` 工具增删改(联调修正:取消固定 4 活动窗口框架);时间段由 LLM 自主排布,窗口不重叠,**允许空白时间**(空白=自由时间)。
