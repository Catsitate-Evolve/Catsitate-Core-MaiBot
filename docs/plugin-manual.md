...
- v0.8.0(2026-09-01):三期 **M3 表达与修正**(`qzone_post` 说说发布/表达润色层(content 模型直写+人设口吻润色)/睡前日记旁路生成+API 直发+醒来延迟回注/self 消息回注/发布主动触发+种子自举/空间见闻当日摘要/点赞通知源C/通知独立于窗口/view_friend_feeds+inspect_image hash/发现层翻页,见 §3.13),即当前公测版本。
...
- **日记(M3 表达,入睡任务旁路生成+API 直发+延迟回注)**:入睡时与次日日程生成同属入睡任务——用旁路 LLM(`qzone_diary` 模板,M3-r2 升 v2 人设前置)以 **bot 本人身份**书写(人设前置为素材首段「bot 人设:…」,与表达润色层同一个人设出口),基于**当日真实素材**(日程活动概览+到期备忘+当日空间见闻+聊天时间线+真实天气)生成 80~200 字第一人称日记(每次生成随机目标字数注入「本次目标约 {n} 字」,n∈[80,200],避免定长模板感;模板的「80~200字」区间表述保留),经发布 API(`do_publish`)直发为说说;旁路 LLM 与发布 API 均不经消息链,**不受睡眠拦截(深夜直发)**;睡眠窗口终点未入睡时由 tick 补执行(每窗口至多一篇)。聊天时间线=真实聊天流当日消息按小时分段(bot 消息标「我:」、他人标昵称、单条截 50 字、总量截 20 条;虚拟流消息不含——日记素材=真实聊天);真实天气=当前快照(无数据省略该行);两者取数失败均告警后省略素材行,不拖垮日记主链路。护栏:模板明令「内容必须完全基于给到的当日素材,不要编造」;空/超 300 字视为输出异常跳过;发布失败不落回注快照(没发出去的日记不该回注成「昨晚发过」的假上下文)。**回注延迟到醒来**:正文+发布时刻存 `qzone_pending_diary.json`,醒态 `sleep_tick` 以 self 消息「我昨晚发布的日记:{前 60 字}」补注进虚拟流(睡眠期注入会被 sleep_gate 拦进回顾缓冲,白注入);补注失败保留快照下轮重试。开关 `diary_enabled`(默认开)。
...
- **旧配置升级(v0.7/v0.8)**:持久化的 `qzone.tool_whitelist` 若含已废弃的 `reply` 或缺 `qzone_comment`/`qzone_reply`/`qzone_post`/`view_friend_feeds`,on_load 会告警提示——需手动更新配置补入(reply 项无效可移除);不更新则虚拟流内无法评论/回复/发说说,全域查看工具不可用。
