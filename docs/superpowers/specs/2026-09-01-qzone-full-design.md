...
优先采用主程序原生记忆摘要：虚拟流是主程序的常规聊天会话，主程序的记忆系统（ChatSummaryWritebackService）已在处理它——当虚拟流消息量达到阈值时自动生成 chat_summary。插件读取该摘要并注入真实聊天流。

若主程序记忆不可直接读取，改用旁路 LLM 摘要：插件定期（虚拟流消息量触发或日程窗口边界触发，非仅入睡时）将虚拟流历史交给旁路 LLM 生成见闻。方法与主程序记忆摘要一致（素材→摘要→存储→注入）。

### 7.3 素材范围

虚拟流当日全部历史（浏览动态 + 通知 + bot 回注的自己的说说/日记/评论），以及 bot 的互动行为（评论/点赞记录）。

### 7.4 注入

生成的见闻存入 inject 框架，真实聊天流注入块输出 `[空间见闻] {digest}`——bot 在群聊/私聊中自然引用空间经历。
