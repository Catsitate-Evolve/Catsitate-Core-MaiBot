...
### Task 8: 执行发言判定(已交付 threshold_met;speak 判定因设计变更废弃)

> **设计变更(2026-08-15 用户裁定)**:主动发言交还主程序——插件不生成话术、不做"是否发言"LLM 判定,只 trigger。Task 8 已交付内容中 `threshold_met` 保留(门槛过滤用);`build_speak_prompt`/`parse_speak_response` 与 llm_provider 的 speak 模板注册废弃,由 Task 11 的 Step 0 统一回退(与 speak/greet 相关测试一并删除)。
...
### Task 11: 插件接线 B —— 日程注入块/窗口 trigger/2.3/提醒兜底(含 Task 8 回退)

**Files:**
- Modify: `plugin.py`(主要接线)
- Modify: `catsitate_core/schedule.py`(删除废弃 speak 函数)
- Modify: `catsitate_core/llm_provider.py`(删除 speak 注册)
- Modify: `catsitate_core/config.py` + `tests/test_config.py`(ScheduleSection 字段调整,见 Task 1 节)
- Modify: `tests/test_schedule.py`(删 speak 测试)
- Test: `tests/test_integration.py`(追加)

**Interfaces:**
- Consumes: `ScheduleGenerator.generate`(T7)、`threshold_met`(T8 保留)、`apply_schedule_edit`(T9)、`current_window/next_window/_materialize_template/DEFAULT_TEMPLATE_SCHEDULE`(T6/7)、`MemoService.due_on`(T2)、`build_favorability_block/LEVELS`(一期)、`self.sleep/_enter_sleep/_sleep_tick/_wake_up/_stream_cache/_persona/_scheduler/_prune_day_keys`(T10)
- Produces(plugin.py 内):`_generate_tomorrow_schedule`(覆盖 T10 占位)、`_schedule_tick`(窗口 trigger)、`_build_proactive_intent(window, stream, fav_row)`、`_try_private_greet`(2.3)、`update_schedule` 工具、`_schedule_block`(注入块)、`_remind_fallback_tick`

**核心设计(trigger 模式)**:插件不 send.text、不生成话术;`self.ctx.maisaka.proactive.trigger(stream_id=..., intent=指示 prompt, reason=..., priority="")`;intent = 模板构建:日程窗口活动/是否计划发言/主题 + 目标流等级注记,要求 bot 结合日程与好感度自然发起主动发言;主程序决定说不说/说什么。每次 trigger 计 1(daily_speak_limit)。

- [ ] **Step 0: 设计变更回退(先于新接线)**

1. `catsitate_core/schedule.py` 删除 `build_speak_prompt`/`parse_speak_response`(保留 `threshold_met` 与 `from .favorability import LEVEL_INDEX`)
2. `catsitate_core/llm_provider.py` 删除 `SIDE_TEMPLATES["speak"]` 注册(Task 8 加的)
3. `tests/test_schedule.py` 删除 `test_build_speak_prompt_window_content_first` 与 `test_parse_speak_response`(保留 test_threshold_met)
4. `catsitate_core/config.py` ScheduleSection:删除 `speak_llm_model`/`speak_llm_timeout_ms`,新增 `speak_max_streams_per_window: int = _f(1, "每窗口最多主动触发流数(按等级+活跃度排序取前 n)", label="每窗口触发流数上限")`;`tests/test_config.py` 同步断言
5. 跑 `python3 -m pytest tests/ -q` 确认全绿,提交:`git commit -m "refactor(phase2): 回退 speak 执行判定(设计变更:主动发言交还主程序,仅保留门槛)"`
...
Run: `python3 -m pytest tests/test_integration.py -q`
Expected: FAIL(NameError: build_proactive_intent)

- [ ] **Step 3: 实现**

1. `catsitate_core/schedule.py` 追加:
...
2. `plugin.py` on_load 追加初始化与调度器注册:

```python
        self._schedule_generated: bool = False  # 当天日程是否为 LLM 生成(模板撑场为 False)
        self._remind_fired: dict[str, str] = {}
...
3. `_generate_tomorrow_schedule`(覆盖 T10 占位):
...
`_active_streams_over(threshold, day)`:从 `self._stream_cache` 取活跃流(该流 `batch_counter.last_bump` 近 24h;查询:`self.store.query("SELECT last_bump FROM batch_counter WHERE stream_id = ?", ...)` 或经 `self.fav_engine` 数据),用 `self.fav_engine.get_level` 的 per-user 等级经 `threshold_met` 过滤,按 (LEVEL_INDEX[level], 最近活动) 降序返回 `[{stream_id, user_id, level_name, note}]`;私聊流 user_id 取流信息 user_id,群聊流 user_id 取最近非 bot 说话人(复用 `_resolve_speaker` 的近似:群聊以流内最近非 bot 用户)。实现时对每个活跃流调用 `_resolve_speaker(stream_id)` 获取当前说话人;空则跳过。

5. `_try_private_greet`(2.3,trigger 模式):
...
6. `update_schedule` 工具(同原 brief,无 window 参数变更):
...
8. `_remind_fallback_tick`(提醒兜底,仅模板撑场时):
...
9. 辅助方法(同原 brief):`_day_overview_text/_weather_text/_fav_summary_text/_today_review_text/_persist_schedule/_prune_day_keys`

10. plugin.py 顶部 import 追加(如未导入):`from catsitate_core.schedule import DEFAULT_TEMPLATE_SCHEDULE, _materialize_template, apply_schedule_edit, build_proactive_intent, current_window, next_window` 与 `from catsitate_core.favorability import LEVELS, threshold_met`(threshold_met 在 schedule.py——按实际位置导入)

- [ ] **Step 4: 跑测试确认通过**

Run: `python3 -m pytest tests/ -q`
Expected: PASS(全量,含回退后测试)

- [ ] **Step 5: 提交**

```bash
git add plugin.py catsitate_core/schedule.py catsitate_core/llm_provider.py catsitate_core/config.py tests/
git commit -m "feat(phase2): 接线日程 trigger 模式(注入块/窗口触发/2.3/工具/提醒兜底;回退 speak 判定)"
```
...
- [ ] **Step 1-2: 创建 4 个文件(内容如下,含占位符者与 build_side_prompt replacements 对应)**
...
git commit -m "feat(phase2): 4 个旁路 prompt 模板(decay/sleep_confirm/schedule_generate/sleep_review)"
```
