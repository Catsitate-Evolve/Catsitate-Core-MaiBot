...
> 实现时首个 spike 验证:插件加载器是否支持子包(插件目录加入 `sys.path` 后绝对导入 `catsitate_core.*`)。若实测不支持,退化为同目录平铺多模块文件,无相对导入。
...
- 唯一入口:`maisaka.planner.before_request` Hook(BLOCKING/LATE,`allow_kwargs_mutation=True`),**只向 `items` 尾部追加** user 消息,绝不改动 system prompt 与历史前缀;
- **注入合并**:所有模块的注入内容合并为**至多一条**尾部 user 消息(单条 `[Catsitate 感知]`,内部小节:环境/好感度/备忘),减少消息条数与缓存边界扰动;
...
**缓存命中优化**(供应商前缀缓存语义下):
- 插件承诺不碰 system prompt 与历史(最长公共前缀不受损);注入在尾部,尾部变化只影响尾部之后的少量 token;
