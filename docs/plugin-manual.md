...
- v0.8.6(2026-09-02):终审修复波+裁定执行(登录态同轮自愈(#7)/bot 昵称直接抛错(#33)/同说说评论硬上限删除/通知发现-入队原子性回退/窗口收尾持泵锁/取消回队防丢在途项/压缩出事件循环+解码防护/失败图占位/mime 魔数探测/单图下载防炸/speak_threshold_level 校验,见 §3.13),即当前公测版本。
