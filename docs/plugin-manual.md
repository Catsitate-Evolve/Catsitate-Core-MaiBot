...
- v0.8.2(2026-09-02):日记深化与输出卫生(日记模板 v6 照搬 diary_plugin 原文(像人的内心描写)/聊天时间线改 message.get_by_time 全天全局拉取/见闻+睡眠回顾 prompt 去机械化/全部可见内容截断尾加"...",见 §3.13),即当前公测版本。
...
- `QQ空间日记发布成功: …`(入睡任务日记直发);`QQ空间日记 LLM 生成失败,跳过本轮` / `QQ空间日记 LLM 失败(success=…),跳过`(旁路生成失败,当夜无日记);`QQ空间日记内容异常(长度=…),跳过发布`(空/超 300 字护栏);`QQ空间日记发布失败(内容已生成,发布跳过)`;`日记聊天素材取数失败(stream=…),该流跳过` / `日记天气素材读取失败,素材行省略`(v0.8.0 日记素材可省略行:单素材失败告警后跳过,不拖垮日记);`QQ空间日记醒来补注完成`;`QQ空间日记补注失败(下个 tick 重试)`(快照保留,醒态 tick 重试)
...
| expression_llm_model | "memory" | 表达润色模型(评论/回复/说说正文按人设口吻润色,task 名;失败以草稿直发) |
...
| 源C 点赞通知无「点赞于」时间或首轮扎堆/迟迟不出现 | 源C「与我相关」流当前无时间锚:新鲜度截断不生效、「点赞于」时间段省略(不编造时间)、上线首轮的存量赞按单轮 3 条上限分轮消化——均属预期非缺陷;持续零事件看有无「源C 连续 3 轮解析 0 条…锚点可能漂移」告警(有则需实机核对响应形态),每轮条数看「源C 赞事件解析 N 条」观测线 |
