...
- **生成 prompt 输入**:人设 + 行为风格(均取主程序 `personality.personality` / `personality.behavior_style` 配置,非硬编码,稳定段顺序固定保前缀缓存)+ 今天执行情况回顾(实际睡了多久/活动执行与否)+ 明天天气/节日 + 重要用户好感度概况 + ****日程对应日**到期的备忘录提醒(remind_at)** + 睡眠约束(min/max)。模板 `catsitate_schedule_generate.prompt`。
