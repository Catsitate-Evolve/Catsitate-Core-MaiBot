...
| 缓存 | 所有注入走统一注入框架,只追加请求尾部;注入内容低频变化;测量用主程序日志/统计 |
...
- 唯一入口:`maisaka.planner.before_request` Hook(BLOCKING/LATE,`allow_kwargs_mutation=True`),**只向 `items` 尾部追加** user 消息,绝不改动头部与历史;
- 每个注入片段由 (模块, 内容 key) 标识,内容未变时渲染结果缓存,保证逐请求字节级稳定;
- 单次注入总长度上限(默认 600 字符,可配),超限按优先级截断(顺序:天气/节日 > 好感度 > 备忘录);
- 失败原则:任一注入源出错仅记录日志并跳过该片段,不阻塞主链路(注入源自身是容错边界,主程序功能不受影响);
...
- 注入:当前流相关的活跃备忘(最近 N 条,默认 3)经注入框架尾部追加:`[备忘] 用户说过:周四要交作业。`;
