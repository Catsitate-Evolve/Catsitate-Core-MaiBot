...
- Produces: `MemoService.write(content, stream_id, user_id, ttl_hours, remind_at="", now=None)`(一期位置签名兼容,追加 remind_at);`MemoService.due_on(day: str, now)` → list[dict](某日到期的备忘);SQLite 列迁移 `ALTER TABLE memo ADD COLUMN remind_at TEXT NOT NULL DEFAULT ''`(幂等:先查 PRAGMA table_info)
...
Expected: FAIL(ModuleNotFoundError: catsitate_core.decay)
...
```python
    async def _window_speak(self, day: str, win: dict) -> None:
        ...
        await self.ctx.send.text(
            text=data["text"].strip()[:200], stream_id=target["stream_id"],
            sync_to_maisaka_history=True,  # SDK SendCapability.text 透传 capability 参数
        )
...
```python
    async def _try_private_greet(self, day: str, win: dict) -> bool:
        ...
        if not text:
            return False
        await self.ctx.send.text(text=text[:200], stream_id=stream_id, sync_to_maisaka_history=True)
