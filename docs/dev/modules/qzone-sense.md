...
2. **运行**:调度器每 tick 触发 `_qzone_poll_tick`——防重入后把长 IO 派发为后台任务 `_qzone_poll_feeds`(不阻塞调度器 60s tick 里的其它任务)。发现层取数统一走 `_qzone_shared_discovery` 入口(**单飞+共享缓存+限流退避**):浏览层与通知源B(默认 300 秒/次的楼中楼检测,`notification_interval_seconds`)原本各自直调同一端点(同口径、首页均无游标),合计约 860 次/天持续触发服务端限流(-10001 network busy);合并为一次请求源后,并发调用经单飞锁只放行一次真实拉取(等待者共享结果),首页列表进实例级共享缓存(`discovery_cache_ttl_seconds`,默认 600 秒,命中即免请求,默认口径下端点调用至多约 144 次/天),撞限流则进入共享退避并按当日次数指数升级(`discovery_backoff_base_minutes`:同日第 1/2 次退避 base/2×base 分钟,默认 30/60;第 3 次起当日熔断至次日零点自动恢复——反复撞墙说明当日预算已超;每次进入打一条 warning,期间两消费方零请求静默返态,退避过期后首次成功真实拉取打 info「限流退避结束,恢复拉取」并复位退避,当日计数按自然日清零不随单次恢复复位)。`discovery_enabled=false` 为发现层**急停阀**:统一入口返回 disabled 态,浏览轮零发现零充实(泵照常推进、发布触发照常结算)、通知源B 跳过,自扫评论与写动作不受影响。全部请求节奏(通知/浏览拉取间隔、页间与好友间 2 秒基准间距)经 `request_jitter_ratio`(默认 0.2,0=关闭)随机抖动——固定到毫秒的周期性是最强机器特征之一;传输指纹目标 `browser_impersonate`(默认 chrome=跟随 curl_cffi 内置最新,可钉具体版本如 chrome131),热重载换目标即重建会话。`QzoneAuthError` 与其他异常原样上抛(浏览层/源B 各自显式告警后跳过本轮);带游标的翻页不经过本层——浏览层积压补全时持与首页列表**同源绑定返回**的续页游标直发(游标随列表经返回值绑定,非锁外读全局态,消除列表与游标不同源的竞态)。配置热重载会失效共享缓存(discovery_count 口径变更)但**不清退避态**(限流是服务端状态,清退避会在风控窗口内重新打 API)。
...
| 发现层服务限流(-10001,QzoneRateLimitError) | 进入共享退避(单条告警)并按当日次数指数升级:第 1/2 次退避 base/2×base 分钟(默认 30/60),第 3 次起当日熔断至次日零点;期间浏览与源B 零请求,期满自动恢复探测;浏览侧另打原文案告警跳过本轮 |
| 发现层急停(`discovery_enabled=false`) | 统一入口返回 disabled:浏览零拉取(泵照常)、源B 跳过;自扫评论与写动作不受影响 |
