...
- 入睡时(与次日日程生成同属入睡任务;睡眠窗口终点未入睡时补执行,每窗口至多一篇)以旁路 LLM、**bot 本人身份**(人设前置为素材首段,模板 `qzone_diary` v2)基于当日真实素材(日程活动/到期备忘/当日空间见闻)生成 80~200 字第一人称日记,经发布 API 直发为说说——不经消息链,不受睡眠拦截(深夜直发);模板明令不得编造素材外内容,空/超 300 字视为输出异常跳过。回注延迟到醒来:正文+发布时刻存 `qzone_pending_diary.json`,醒态以 self 消息补注进虚拟流,失败保留下轮重试。
...
- **日程窗口属性分离**:活动窗口的 `qzone` 单属性拆为 `read_qzone`(刷空间)与 `send_qzone`(发说说)两个独立标记(可同窗,仅 daily 合法;旧 `qzone` 键读取兼容),日程生成模板升 v4。
