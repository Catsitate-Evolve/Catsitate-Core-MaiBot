...
- **通知轮询**:`qzone_notify_poll` 调度任务(默认 300 秒,注册下限 30s),始终运行、醒着即可——通知是推送语义,不隶属任何浏览窗口。
...
- **源A(自己说说下的新评论,含 bot 参与线程的楼中楼回复)**:`get_own_feed_comments`(单次 msglist 请求三用:评论映射 + 正文上下文 + 楼中楼回复,第三视图同载荷补跑 `parse_feed_replies`,不发第二次请求)。逐条以 `feed_tid:comment_tid:uin` 为去重键 `is_new` 登记判新;自己发出的评论重见即幂等登记不注入;早于 `summary_days`(默认 3 天)的过旧通知跳过。正文=「评论了你的说说:…」+ 参数行;楼中楼回复正文=「回复了你的评论「{bot 在该线程最近的发言前20字}」:…」(与源B 同形态,去重键 `{feed_tid}:{parent_comment_tid}:reply:{reply_tid}`)。**线程筛选=bot 参与过**(实机抓包实证 2026-09-04:QQ 楼中楼一律挂**顶层评论**的 list_3,与回复谁无关——「好友评论→bot 楼中楼回复→好友再回复」线程的顶层作者是好友,按顶层作者==bot 筛选会漏);通知二元组锚=顶层评论(id+作者,与 `qzone_reply` 发送形态一致)。好友回复另一好友、bot 未参与的旁听线程不通知(bot 不插话他人对话)。源A 服务限流(-10001)时本轮源A 按空处理,**不阻断源B/C**(各源取数与失败处置相互隔离)。
- **源B(自己在他人说说下的评论线程收到楼中楼回复)**:搭发现层便车——本地反查近 30 天 bot 评论过的好友(`bot_commented_friends`),与统一时间线单页的活跃作者取交集,只对「有新活动且评论过」的好友拉原始载荷(`get_user_feeds_raw`),`parse_feed_replies` 解析 bot 参与线程的 list_3(筛选语义同源A:顶层评论是 bot 的,或 list_3 中有 bot 的回复)。零交集时零源B 拉取。正文=「回复了你的评论「{bot 在该线程最近的发言前20字}」:…」+ 参数行;二元组锚=顶层评论(id+作者)。
