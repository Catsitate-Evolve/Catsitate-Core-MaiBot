...
> 对应代码:`tests/`(33 个文件,599 个用例,全量离线运行约 6 秒)。
...
- **组合层**(plugin 接线):离线装配插件实例(`_make_plugin`),客户端/LLM/网关全桩,断言「接线行为」——工具目标解析、频控回执、AuthError 自愈、通知轮询守卫等。**断言的是行为,不是源码字符串**。
...
# 599 passed in ~6s(无需网络、无需主程序、无需 QQ 登录态)
```
