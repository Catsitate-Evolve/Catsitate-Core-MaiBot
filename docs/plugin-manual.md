...
- v0.8.0(2026-09-01):三期 **M3 表达与修正**(`qzone_post` 说说发布/表达润色层(content 模型直写+人设口吻润色)/睡前日记旁路生成+API 直发+醒来延迟回注/self 消息回注/发布主动触发+种子自举/空间见闻当日摘要/点赞通知源C/通知独立于窗口/view_friend_feeds+inspect_image hash/发现层翻页,见 §3.13)。
- v0.8.1(2026-09-02):联调改进集(日记模板 v5 仿 diary_plugin 蓝本重排/`qzone_like`+`qzone_comment`+`qzone_post` 三工具全域可用+白名单只管非 qzone 工具/view_friend_feeds 返回格式可读性优化/通知消息不再强制触发回复概率,见 §3.13)。
- v0.8.2(2026-09-02):日记深化与输出卫生(日记模板 v6 照搬 diary_plugin 原文(像人的内心描写)/聊天时间线改 message.get_by_time 全天全局拉取/见闻+睡眠回顾 prompt 去机械化/全部可见内容截断尾加"...",见 §3.13)。
- v0.8.3(2026-09-02):配置固化与日志收敛(虚拟伪群号/显示名固化常量/源C 常规观测日志移除/润色模型默认 replyer/日记超长硬上限移除,见 §3.13)。
- v0.8.4(2026-09-02):截断清理与回执语义(回注/登记截断全删(用户逐项裁定)/润色层不截断/见闻叙事 100 字/-10049 频控回执/get_by_time list 兼容,见 §3.13)。
- v0.8.5(2026-09-02):通知死锁修复+润色事实保持+日程 RPC 超时(awaiting 卡死吞新评论的扫描死锁/润色模板 v6 不修改关键事实/schedule_llm_timeout_ms 默认 120000,见 §3.13)。
- v0.8.6(2026-09-02):终审修复波+裁定执行(登录态同轮自愈(#7)/bot 昵称直接抛错(#33)/同说说评论硬上限删除/通知发现-入队原子性回退/窗口收尾持泵锁/取消回队防丢在途项/压缩出事件循环+解码防护/失败图占位/mime 魔数探测/单图下载防炸/speak_threshold_level 校验,见 §3.13)。
- v0.9.0(2026-09-02):**说说详情工具与 reply 全域化**(view_friend_feed_detail 完整评论查看(楼中楼展开/Q9 上限/查看即已见)/qzone_reply 全域+comment_map 三级解析/view_friend_feeds 页码翻页/浏览注入带评论区(零新增 API)/删 recent_comments 死字段与 routing.py 死代码,见 §3.13)。
- v0.9.1(2026-09-03):**发现层协议校准**(scope=0→2 好友动态流(旧取值实为 2 天小窗口且只见自己动态)/begin 偏移→begintime 游标翻页(旧参数被服务端无视)/解析 appid 回退 appiconid 防窗口边界丢条,见 §3.13)。
- v0.9.2(2026-09-03):终审修复波(reply 通知上下文仅锚匹配采用(错靶修复)/评论区截断标签按显示数/空评论区诚实提示/comment_map 同键昵称保留,见 §3.13)。
- v0.9.3(2026-09-03):第三次全量复审修复波+图片链拼接化(昵称抛错点安全区(发布前读/补注种子入 try)/normalize_ts 与衰减防重入/日记发布同轮自愈/mark_seen 三态保锚/源C 时间折算防护+调用隔离/**图片公共管线:多图拼接合成(序号角标)替代逐张入管线,VLM O(N)→O(1),≤3 截断删除**/锚 hash 按拟合后字节/owner_nickname 语义修正,见 §3.13),即当前公测版本。
