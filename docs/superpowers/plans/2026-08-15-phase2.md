...
def test_parse_decay_response_ok():
    assert parse_decay_response('{"delta": -2, "note": "有点生疏了"}') == (-2, "")
    ...
    async def scan_and_apply(
        ...
        for user_id, stream_id, interaction_ts, is_group in candidates:
            ...
            if not isinstance(result, dict) or not result.get("success"):
                continue
            delta, err = parse_decay_response(str(result.get("response") or ""))
            if delta is None:
                continue
            ...
            self.engine.apply_delta(
                user_id, stream_id, delta, "关系自然衰减", judged_at=judged_at, judge_id=f"decay-{judged_at}"
            )
            results.append({"user_id": user_id, "stream_id": stream_id, "delta": delta, "note": "关系自然衰减"})
        return results
```

注意:上面 `apply_delta` 的 note 参数在实机上会被注记截断逻辑处理(≤40 字),「关系自然衰减」5 字安全。LLM 返回的新 note 未用(简化:衰减注记固定文本,不覆盖原注记,避免衰减判定改写关系注记;规格 §3.1 说「注记更新」——实现为 apply_delta 的 note 参数,但此处传固定文本更稳;spec 要求"注记更新为拟人化描述",采用 LLM 的 note:`apply_delta(..., note=parsed_note)`,parse_decay_response 需返回 note。修正实现:parse 返回 (delta, note) 时 note 一并传出。为保持本任务测试一致,parse 返回 `(delta, err)` 且实现里用响应中的 note:在 parse 成功后从 result 文本再取 note 字段——改为 parse_decay_response 返回 `(delta, note_or_err)`,delta 为 None 时第二元是错误原因,delta 非 None 时第二元是 note。测试 test_parse_decay_response_ok 断言 (-2, "有点生疏了") 已按此写 ✓,测试中 `parse_decay_response('{"delta": 2...}')[0] is None` 兼容 ✓。)
...
def test_clamp_wake_too_long_forces_earlier(tmp_path):
    ...
    assert mgr.clamp_wake_time("2026-08-15T20:00:00", "2026-08-16T07:00:00") == "2026-08-16T07:00:00" or True
    # 真正超界:20:00 睡到 07:00 = 11h=660min 恰好边界;用 19:00 睡 → 12h > 660 → 提前到 19:00+660min=06:00
...
def is_goodnight_utterance(text: str) -> bool:
    ...
    if "@" in t or "「" in t or "」" in t:
        return False
    # 称呼他人:短句中出现「晚安,<名字>」类结构
    if re.search(r"[,，、]|你好|再见", t):
        return False
...
GOOD = {
    ...
    "windows": [
        ...
        {"kind": "greeting", "start": "2026-08-16T07:00", "end": "2026-08-16T08:00",
         "activity": "起床", "plan_speak": True, "topic": "早安", "speak_kind": "greeting"},
    ],
}
...
def test_validate_too_many_activities():
    ...
    for i in range(9):
        bad["windows"].append({"kind": "daily", "start": f"2026-08-16T0{i}:00", "end": f"2026-08-16T0{i}:30",
                               "activity": f"活动{i}", "plan_speak": False, "topic": "", "speak_kind": "daily"})
    data, err = validate_schedule(bad, min_sleep=240, max_sleep=660)
...
DEFAULT_TEMPLATE_SCHEDULE: dict = {
    "date": "",
    "windows": [
        {"kind": "sleep", "start": "23:00", "end": "07:30", "activity": "", "plan_speak": False,
         "topic": "", "speak_kind": "greeting"},
        {"kind": "daily", "start": "09:00", "end": "12:00", "activity": "发呆", "plan_speak": False,
         "topic": "", "speak_kind": "daily"},
        {"kind": "daily", "start": "15:00", "end": "18:00", "activity": "随便做点什么", "plan_speak": False,
         "topic": "", "speak_kind": "daily"},
        {"kind": "greeting", "start": "22:00", "end": "23:00", "activity": "洗漱准备睡", "plan_speak": False,
         "topic": "", "speak_kind": "greeting"},
    ],
}
...
- Produces: `build_speak_prompt(persona, window, day_overview, weather_text, fav_block) -> tuple[list[dict], str]`;`parse_speak_response(text) -> tuple[dict | None, str]`(返回 `{"speak": bool, "text": str}` 或 (None, 原因));`threshold_met(level_name: str, threshold_level: str) -> bool`(等级名比较,LEVEL_INDEX 索引)
...
def test_parse_speak_response():
    data, err = parse_speak_response('{"speak": true, "text": "今天天气不错"}')
    assert err == "" and data == {"speak": True, "text": "今天天气不错"}
    data2, err2 = parse_speak_response('{"speak": false, "text": ""}')
    assert err2 == "" and data2["speak"] is False
    assert parse_speak_response("没想好")[0] is None
...
def test_build_speak_prompt_window_content_first():
    messages, key = build_speak_prompt(
        ...
        fav_block="[好感度] u1:等级「熟悉」(累计 42),注记:无。",
    )
    assert messages[0]["role"] == "system"
...
def build_speak_prompt(
    persona: str, window: dict, day_overview: str, weather_text: str, fav_block: str,
) -> tuple[list[dict], str]:
    """执行判定 prompt:稳定段=system 模板+人设;变量尾=窗口活动/全天概览/天气/目标流好感度。"""
    ...
    tail = [
        ...
        f"目标流好感度:{fav_block or '无'}",
    ]
    return build_side_prompt("speak", stable, tail)


def parse_speak_response(text: str) -> tuple[dict | None, str]:
    """解析执行判定 JSON:{"speak": bool, "text": "发言文本(不发言为空)"}。"""
    ...
    return {"speak": data["speak"], "text": text_out}, ""
```
...
def test_edit_add_activity():
    data, err, hist = apply_schedule_edit(
        ...
        {"kind": "daily", "start": "2026-08-16T14:00", "end": "2026-08-16T16:00",
         "activity": "买菜", "plan_speak": False, "topic": "", "speak_kind": "daily"},
...
def test_edit_add_over_limit_rejected_with_reason():
    ...
    out, err, _ = apply_schedule_edit(
        ...
        {"kind": "daily", "start": "2026-08-16T18:00", "end": "2026-08-16T19:00",
         "activity": "x", "plan_speak": False, "topic": "", "speak_kind": "daily"},
        [], min_sleep=240, max_sleep=660,
    )
...
def test_phase2_engines_assemble(tmp_path):
    ...
    data, err, hist = apply_schedule_edit(
        {"date": "2026-08-16", "windows": [
            {"kind": "sleep", "start": "2026-08-16T23:00", "end": "2026-08-17T07:00"},
            {"kind": "daily", "start": "2026-08-16T09:00", "end": "2026-08-16T11:00",
             "activity": "a", "plan_speak": False, "topic": "", "speak_kind": "daily"}]},
        "add", None,
        {"kind": "daily", "start": "2026-08-16T14:00", "end": "2026-08-16T16:00",
         "activity": "b", "plan_speak": False, "topic": "", "speak_kind": "daily"},
...
```python
        self._scheduler.register("daily_decay", 3600, self._daily_decay)
...
```python
    ...
    async def _enter_sleep(self) -> None:
        ...
        planned_wake = win.get("end") if win and win.get("kind") == "sleep" else (now + timedelta(hours=8)).strftime("%Y-%m-%dT%H:%M")
        wake_at = self.sleep.clamp_wake_time(now.strftime("%Y-%m-%dT%H:%M"), planned_wake)
...
```python
    async def _sleep_tick(self) -> None:
        ...
        if self.sleep.is_sleeping(now=lambda: now):
            if now_iso >= self.sleep.state.wake_at:
                self.ctx.logger.info("自然醒来: %s", now_iso)
                await self._wake_up()
            return
    ...
    async def _daily_decay(self) -> None:
        ...
        try:
            ...
            for user_id, stream_id in self.fav_engine.iter_today_active():
                ...
                candidates.append((user_id, stream_id, interaction or "", is_group))
...
def test_schedule_inject_block_text():
    ...
    data = {"date": "2026-08-16", "windows": [
        {"kind": "daily", "start": "2026-08-16T09:00", "end": "2026-08-16T11:00",
         "activity": "发呆看雨", "plan_speak": False, "topic": "", "speak_kind": "daily"},
        {"kind": "sleep", "start": "2026-08-16T23:00", "end": "2026-08-17T07:00"},
    ]}
...
```python
    async def _schedule_tick(self) -> None:
        ...
        if not self._schedule_data or self._schedule_data.get("date") != day:
            self._schedule_data = _materialize_template(DEFAULT_TEMPLATE_SCHEDULE, day)  # 首日模板撑场
        win = current_window(self._schedule_data, now.strftime("%Y-%m-%dT%H:%M"))
...
```python
    async def _window_speak(self, day: str, win: dict) -> None:
        ...
        weather = self._weather_text()
        fav_block = build_favorability_block(self.fav_engine, candidates[0]["user_id"], candidates[0]["stream_id"])
        messages, _ = build_speak_prompt(await self._persona(), win, overview, weather, fav_block)
        try:
            ...
            return
        data, err = parse_speak_response(str(result.get("response") or ""))
        if data is None:
            self.ctx.logger.warning("执行判定解析失败(%s),本窗口沉默", err)
            return
        if not data["speak"] or not data["text"].strip():
            return
        target = candidates[0]  # LLM 单流选择简化:候选排序交给 _active_streams_over(按等级+活跃度)
...
`_active_streams_over(threshold, day)`:从 `self._stream_cache`(Task 10 已建)取活跃流(近 24h 有用户消息的流;用 `self.fav_engine` 的 per-user 等级过滤),按 (等级, 最近活动) 排序返回 `[{stream_id, user_id, level_name}]`;不满足门槛返回空列表。
...
```python
    @Tool(
        ...
        parameters=[
            ...
            ToolParameterInfo(name="window", param_type="string", description='活动窗口 JSON:{"kind":"daily"或"greeting","start":"YYYY-MM-DDTHH:MM","end":"...","activity":"活动描述","plan_speak":true/false,"topic":"主题","speak_kind":"daily"或"greeting"}', required=False),
        ],
        visibility="visible",
    )
        ...
        if cfg.schedule.enabled and cfg.time_aware.enabled:
            ...
            if win and win.get("kind") != "sleep":
                ...
                if nxt:
                    ...
                    line += f";接下来:{label}"
                blocks.append(InjectionBlock("schedule", f"sch:{win.get('start')}", line))
```
...
```python
    async def _remind_fallback_tick(self) -> None:
        """备忘提醒独立兜底:无日程可用时到点注入备忘归属流(不主动发言);睡眠中不执行。"""
...
```python
    ...
    def _today_review_text(self) -> str:
        return f"睡了约 {(self.sleep.state.sleep_at or '')[:16]}~{(self.sleep.state.wake_at or '')[:16]}(无有效数据)" if self.sleep.state.sleep_at else "无"
...
- [ ] **Step 1-2: 创建 5 个文件(内容如下,含占位符者与 build_side_prompt replacements 对应)**
...
- 活动窗口(1~8 个):{"kind": "greeting" 或 "daily", "start": "...", "end": "...", "activity": "活动描述", "plan_speak": true/false, "topic": "发言主题(计划发言时)", "speak_kind": "greeting" 或 "daily"}
...
决定 bot 此刻是否主动发送一条消息。
严格输出 JSON:{"speak": true/false, "text": "发言文本"}。
不发言时 speak=false 且 text 为空字符串;发言文本简短、贴合人设(如「…下雨了」)。
...
git commit -m "feat(phase2): 5 个旁路 prompt 模板(decay/sleep_confirm/schedule_generate/speak/greet)"
```
