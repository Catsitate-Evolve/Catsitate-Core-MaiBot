...
> **联调修正波（8291284..c5b0232）已偏离本文多处（拉取架构/时间戳语义/图片组件形态等），本文为历史执行记录，现行语义以 spec 为准。**
...
(split 后第 2 段,如 qzone-qq → qq)计算命名空间 person_id,person 因此与真实
QQ 聊天统一为同一人(spec §2.17)。
"""
...
- Produces: plugin 实例属性 `self._qzone_session_ids: set[str]` 与方法 `def _qzone_session_id_set(self) -> set[str]`（fav_count 内联 is_qzone_message 判定+session 收集，无独立豁免方法）。
