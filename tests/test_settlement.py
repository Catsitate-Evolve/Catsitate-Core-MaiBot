...
def test_favorability_block_render(tmp_path):
    ...
    engine.apply_delta("u1", 42, "最近主动关心过你", judged_at=dt.now().strftime("%Y-%m-%dT%H:%M:%S"))
    text = build_favorability_block(engine, "u1")
...
def test_favorability_block_default_stranger(tmp_path):
    executor, engine, _ = make_executor(tmp_path)
    text = build_favorability_block(engine, "newbie")
...
def test_favorability_block_include_rule_single_level(tmp_path):
    ...
    from datetime import datetime as dt
    engine.apply_delta("u1", 42, "最近主动关心过你", judged_at=dt.now().strftime("%Y-%m-%dT%H:%M:%S"))
    text = build_favorability_block(engine, "u1", include_rule=True)
    lines = text.split("\n")
...
def test_favorability_block_include_rule_stranger(tmp_path):
    executor, engine, _ = make_executor(tmp_path)
    text = build_favorability_block(engine, "newbie", include_rule=True)
