...
7. **好感度以人为唯一标识(联调裁定 2026-08-16)**:用户身份 = QQ 号(user_id)。`favorability` 单行按人存储与判定(跨流聚合),不以 (user_id, stream_id) 分流;日终结算按人**合并为一次**(素材聚合该人当日所有流的消息,群聊只取该人发言与 bot 对其 quote/@ 回应);衰减计时按人跨流取最近互动;开发阶段不做旧数据迁移(改 schema 直接重建);`batch_counter` 保留 (user, stream) 行仅作活跃度记录,结算窗口改用人级 `favorability.window_start`;2.3 主动私聊仅当该人存在私聊流时适用,无私聊流则跳过(该功能不适用于该人,不群内替代)。
...
- 仅在 **kind=greeting 的窗口**触发,执行顺序:**先 2.3 后 2.1**(防双触发):greeting 窗口到达时,先检查**存在私聊流**且好感度(按人)≥ `private_threshold_level`(默认挚友)、当日未触发过的用户——存在则对其私聊流 trigger(问候语境指示 prompt,含日程活动与全天概览,问候话术由主程序生成),每人每日最多一次(计 1);已 trigger 则该窗口跳过 2.1;无符合条件的候选时再走 2.1(候选流含群/私聊)。**该人无私聊流则 2.3 不适用**(跳过,不群内替代)。
- 睡眠/静默期跳过;被拦截/失败显式日志。
